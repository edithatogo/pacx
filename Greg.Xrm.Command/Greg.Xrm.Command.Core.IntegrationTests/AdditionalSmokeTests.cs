...
namespace Greg.Xrm.Command.IntegrationTests
{
	...
	public class AdditionalSmokeTests : IntegrationTestBase
	{
		...
		public void SmokeTestColumnCreateDeleteShouldRoundTrip()
		{
			...
			try
			{
				...
				column["displayname"] = new Microsoft.Xrm.Sdk.Label("Test Column", 1033);
		...
		[TestMethod]
		[TestCategory("Integration")]
		public void SmokeTestRelationshipCreateDeleteShouldRoundTrip()
		{
			if (!IsConnected) return;
		...
		[TestMethod]
		[TestCategory("Integration")]
		public void SmokeTestWebResourceCreateDeleteShouldRoundTrip()
		{
			if (!IsConnected) return;
		...
		[TestMethod]
		[TestCategory("Integration")]
		public void SmokeTestTimeoutConfigurationShouldRespectTimeout()
		{
			if (!IsConnected) return;

			// Verify ServiceClient has reasonable timeout
			Assert.IsTrue(ServiceClient.MaxConnectionTimeout.TotalSeconds >= 120, "Default timeout should be at least 120 seconds");
			TestContext.WriteLine($"ServiceClient timeout: {ServiceClient.MaxConnectionTimeout.TotalSeconds}s");
		}
		...
		private Guid CreateTestTable(string tableName)
		{
			...
			table["displayname"] = new Microsoft.Xrm.Sdk.Label($"Test Table {tableName}", 1033);
