namespace Greg.Xrm.Command.Commands.AiBuilder
{
	...
	public class AiBuilderCommandsTest
	{
		...
		public void AiModelTrain_ParseWithRequiredShouldWork()
		{
			...
			Assert.AreEqual(5, command.PollIntervalSeconds);
			Assert.AreEqual(600, command.TimeoutSeconds);
		}
		...
		[TestMethod]
		public void AiModelTrain_ParseWithPollingOptionsShouldWork()
		{
			var command = Utility.TestParseCommand<AiModelTrainCommand>(
				"ai", "model", "train",
				"-m", "model-id-123",
			...
			Assert.IsTrue(command.Wait);
			Assert.AreEqual(7, command.PollIntervalSeconds);
			Assert.AreEqual(120, command.TimeoutSeconds);
		}
		...
		[TestMethod]
		public void AiFormProcessor_ParseWithCommaSeparatedListsShouldWork()
		{
			var command = Utility.TestParseCommand<AiFormProcessorConfigureCommand>(
				"ai", "form-processor", "configure",
				"-m", "model-id-123",
				"-d", "Invoice",
				"-f", "TotalAmount,InvoiceDate,VendorName",
				"-t", "LineItems,Attachments");

			Assert.AreEqual("model-id-123", command.ModelId);
			Assert.AreEqual("Invoice", command.DocumentType);
			Assert.AreEqual(3, command.Fields?.Length);
			Assert.AreEqual("TotalAmount", command.Fields?[0]);
			Assert.AreEqual("InvoiceDate", command.Fields?[1]);
			Assert.AreEqual("VendorName", command.Fields?[2]);
			Assert.AreEqual(2, command.Tables?.Length);
			Assert.AreEqual("LineItems", command.Tables?[0]);
			Assert.AreEqual("Attachments", command.Tables?[1]);
		}
	}
}
