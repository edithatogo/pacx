...
# Customize polling
pacx ai model train --model-id "model-guid-123" --wait --poll-interval 10 --timeout 900
```
...
| `--wait` | `-w` | Wait for training to complete | false |
| `--poll-interval` | | Polling interval in seconds when `--wait` is used | 5 |
| `--timeout` | | Timeout in seconds when `--wait` is used | 600 |
