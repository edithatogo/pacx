...
namespace Greg.Xrm.Command.Commands.Auth
{
		...
		IPacxProjectRepository pacxProjectRepository) : ICommandExecutor<ListCommand>
	{
		...
		{
			var connections = await organizationServiceRepository.GetAllConnectionDefinitionsAsync().ConfigureAwait(false);
			if (connections.Count == 0)
			{
				return CommandResult.Fail("No authentication profiles found.");
			}

			var project = await pacxProjectRepository.GetCurrentProjectAsync().ConfigureAwait(false);
			...
			foreach (var item in connections.ConnectionStringKeys)
			{
				...
				var environmentName = await organizationServiceRepository.GetEnvironmentFromConnectioStringAsync(item).ConfigureAwait(false);
