...
namespace Greg.Xrm.Command.Commands.Column.Create
{
		...
		where TCommand : BaseCreateCommand
	{
		...
		protected virtual async Task<CommandResult> ExecuteAsync(
			TCommand command,
			Func<IOrganizationServiceAsync2, TCommand, int, string, int, CancellationToken, Task<AttributeMetadata>> createAttributeFunc,
			CancellationToken cancellationToken)
		{
			...
			try
			{
				var defaultLanguageCode = await crm.GetDefaultLanguageCodeAsync().ConfigureAwait(false);


				var (publisherPrefix, currentSolutionName, customizationOptionValuePrefix) = await CheckSolutionAndReturnPublisherPrefixAsync(crm, command.SolutionName, cancellationToken).ConfigureAwait(false);
				...
				var attribute = await createAttributeFunc(crm, command, defaultLanguageCode, publisherPrefix, customizationOptionValuePrefix.Value, cancellationToken).ConfigureAwait(false);
				...
				var response = (CreateAttributeResponse)await crm.ExecuteAsync(request, cancellationToken).ConfigureAwait(false);
		...
		private async Task<(string?, string?, int?)> CheckSolutionAndReturnPublisherPrefixAsync(IOrganizationServiceAsync2 crm, string? currentSolutionName, CancellationToken cancellationToken = default)
		{
			...
			var solutionList = (await crm.RetrieveMultipleAsync(query, cancellationToken).ConfigureAwait(false)).Entities;
