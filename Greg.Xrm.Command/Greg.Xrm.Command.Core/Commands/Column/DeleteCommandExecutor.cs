...
namespace Greg.Xrm.Command.Commands.Column
{
			...
			: ICommandExecutor<DeleteCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(DeleteCommand command, CancellationToken cancellationToken)
		{
			...
			var columnMetadata = await RetrieveColumnMetadataAsync(crm, command, cancellationToken).ConfigureAwait(false);
			...
			var dependencies = await RetrieveDependenciesAsync(crm, command, columnMetadata, cancellationToken).ConfigureAwait(false);
			...
			try
			{
				...
				else if (dependencies.Count > 0)
				{
					...
					{
						// Force delete
						await attributeDeletionService.DeleteAttributeAsync(crm, columnMetadata, dependencies, cancellationToken: cancellationToken).ConfigureAwait(false);
		...
		private async Task<DependencyList?> RetrieveDependenciesAsync(IOrganizationServiceAsync2 crm, DeleteCommand command, AttributeMetadata columnMetadata, CancellationToken cancellationToken)
		{
			try
			{
				...
				var dependencyList = await dependencyRepository.GetDependenciesAsync(crm, ComponentType.Attribute, columnMetadata.MetadataId.GetValueOrDefault(), true, cancellationToken).ConfigureAwait(false);
		...
		private async Task<AttributeMetadata?> RetrieveColumnMetadataAsync(IOrganizationServiceAsync2 crm, DeleteCommand command, CancellationToken cancellationToken)
		{
			try
			{
				...
				var response = (RetrieveAttributeResponse)await crm.ExecuteAsync(request, cancellationToken).ConfigureAwait(false);
