	...
	public class GetDependenciesCommandExecutor : ICommandExecutor<GetDependenciesCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(GetDependenciesCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				var dependencies = await this.dependencyRepository.GetDependenciesAsync(crm, ComponentType.Attribute, attribute.MetadataId.GetValueOrDefault(), command.ForDelete, cancellationToken).ConfigureAwait(false);
