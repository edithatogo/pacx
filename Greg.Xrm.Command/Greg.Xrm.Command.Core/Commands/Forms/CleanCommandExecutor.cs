...
namespace Greg.Xrm.Command.Commands.Forms
{
			...
			ISolutionRepository solutionRepository) : ICommandExecutor<CleanCommand>
	{
		public async Task<CommandResult> ExecuteAsync(CleanCommand command, CancellationToken cancellationToken)
		{
			...
			var formList = await formRepository.GetMainFormByTableNameAsync(crm, command.TableName).ConfigureAwait(false);
			output.WriteLine("Done", ConsoleColor.Green);
			...
			var languageCode = await crm.GetDefaultLanguageCodeAsync().ConfigureAwait(false);
			output.WriteLine("DONE, default language code is " + languageCode, ConsoleColor.Green);

			var table = await this.RetrieveEntityMetadataAsync(crm, command.TableName, cancellationToken).ConfigureAwait(false);



			var (success, result1, solution) = await CreateHoldingSolutionAsync(crm, command.SolutionName, cancellationToken).ConfigureAwait(false);
			...
			using (solution)
			{
				try
				{
					// add the form to the temporary solution
						await solution.AddComponentAsync(form.Id, ComponentType.SystemForm, cancellationToken).ConfigureAwait(false);

					// now I need to download the solution, extract the customizations.xml file, modify it, and re-upload it
						using (var solutionContent = await solution.DownloadAsync(cancellationToken).ConfigureAwait(false))
					{
						...
						if (hasChanges)
						{
							await solution.UploadAndPublishAsync(newZipBytes, cancellationToken, command.TableName).ConfigureAwait(false);
						}
						else
						{
		...
		private async Task<(bool, CommandResult?, ITemporarySolution?)> CreateHoldingSolutionAsync(IOrganizationServiceAsync2 crm, string? currentSolutionName, CancellationToken cancellationToken)
		{
		...
		private async Task<EntityMetadata> RetrieveEntityMetadataAsync(IOrganizationServiceAsync2 crm, string tableName, CancellationToken cancellationToken)
		{
			...
			var response = (RetrieveEntityResponse)await crm.ExecuteAsync(request, cancellationToken).ConfigureAwait(false);
			output.WriteLine("Done", ConsoleColor.Green);
