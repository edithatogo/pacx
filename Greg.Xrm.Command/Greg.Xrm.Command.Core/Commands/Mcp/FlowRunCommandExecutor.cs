...
{
	public sealed class FlowRunCommandExecutor(IOutput output) : ICommandExecutor<FlowRunCommand>
	{
		public Task<CommandResult> ExecuteAsync(FlowRunCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				if (!string.IsNullOrWhiteSpace(flow.HomePage))
				{
					...
					{
						Process.Start(new ProcessStartInfo(flow.HomePage)
						{
				...
				{
					return Task.FromResult(CommandResult.Fail($"Flow entry <{flow.Name}> does not declare a homepage to open."));
				}
			...
			{
				return Task.FromResult(CommandResult.Fail($"Unable to run flow entry from <{command.CatalogPath}>: {ex.Message}", ex));
			}
		}
	}
}
