...
namespace Greg.Xrm.Command.Commands.OptionSet
{
	...
	: ICommandExecutor<AddCommand>
	{
			...
			try
			{
				...
				if (!isStatusCode)
				{
					...
					if (!value.HasValue)
					{
						...
						value = await GenerateNextValueAsync(crm, command, cancellationToken).ConfigureAwait(false);
				...
				await crm.ExecuteAsync(publishXmlBuilder.Build(), cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
		...
		{
			var optionSetValues = await GetCurrentValuesAsync(crm, command, cancellationToken).ConfigureAwait(false);
		...
		private async Task<IReadOnlyCollection<OptionMetadata>> GetCurrentValuesAsync(IOrganizationServiceAsync2 crm, AddCommand command, CancellationToken cancellationToken)
		{
			if (!string.IsNullOrWhiteSpace(command.Name))
			{
				...
				var response = (RetrieveOptionSetResponse)await crm.ExecuteAsync(request, cancellationToken).ConfigureAwait(false);
