		...
		) : ICommandExecutor<InfoCommand>
	{
		public async Task<CommandResult> ExecuteAsync(InfoCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				var response = (RetrieveCurrentOrganizationResponse)await crm.ExecuteAsync(request, cancellationToken).ConfigureAwait(false);
				...
				org = (await crm.RetrieveMultipleAsync(query, cancellationToken).ConfigureAwait(false)).Entities[0];
