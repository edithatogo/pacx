		...
		) : ICommandExecutor<ListAvailableLanguagesCommand>
	{
		public async Task<CommandResult> ExecuteAsync(ListAvailableLanguagesCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				languageList = (await crm.RetrieveMultipleAsync(query, cancellationToken).ConfigureAwait(false)).Entities.ToList();
			...
			try
			{
				...
				var response2 = (RetrieveProvisionedLanguagesResponse)await crm.ExecuteAsync(request2, cancellationToken).ConfigureAwait(false);
