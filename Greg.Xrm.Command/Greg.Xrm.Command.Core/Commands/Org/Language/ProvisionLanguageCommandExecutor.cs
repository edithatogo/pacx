		...
		) : ICommandExecutor<ProvisionLanguageCommand>
	{
		public async Task<CommandResult> ExecuteAsync(ProvisionLanguageCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				var org = (await crm.RetrieveMultipleAsync(query2, cancellationToken).ConfigureAwait(false)).Entities[0];
				...
				var response = (RetrieveInstalledLanguagePacksResponse)await crm.ExecuteAsync(request, cancellationToken).ConfigureAwait(false);
			...
			try
			{
				...
				var response = (ProvisionLanguageAsyncResponse)await crm.ExecuteAsync(request, cancellationToken).ConfigureAwait(false);
				...
				while (true)
				{
					...
					var asyncOperation = await crm.RetrieveAsync("asyncoperation", asyncOperationId, new ColumnSet("statecode", "statuscode")).ConfigureAwait(false);
