		...
		IOrganizationServiceRepository organizationServiceRepository) : ICommandExecutor<PagesSiteConfigExportCommand>
	{
		public async Task<CommandResult> ExecuteAsync(PagesSiteConfigExportCommand command, CancellationToken cancellationToken)
		{
			try
			{
				...
				var siteResults = await crm.RetrieveMultipleAsync(siteQuery, cancellationToken).ConfigureAwait(false);
				...
				{
					case "all":
						await ExportAllAsync(crm, site.Id, command.OutputPath, command.Format, cancellationToken).ConfigureAwait(false);
						break;
					case "auth":
						await ExportAuthAsync(crm, site.Id, command.OutputPath, command.Format, cancellationToken).ConfigureAwait(false);
						break;
					case "navigation":
						await ExportNavigationAsync(crm, site.Id, command.OutputPath, command.Format, cancellationToken).ConfigureAwait(false);
						break;
					case "themes":
						await ExportThemesAsync(crm, site.Id, command.OutputPath, command.Format, cancellationToken).ConfigureAwait(false);
						break;
					case "snippets":
						await ExportSnippetsAsync(crm, site.Id, command.OutputPath, command.Format, cancellationToken).ConfigureAwait(false);
						break;
					default:
		...
		{
			await ExportAuthAsync(crm, siteId, outputPath, format, ct).ConfigureAwait(false);
			await ExportNavigationAsync(crm, siteId, outputPath, format, ct).ConfigureAwait(false);
			await ExportThemesAsync(crm, siteId, outputPath, format, ct).ConfigureAwait(false);
			await ExportSnippetsAsync(crm, siteId, outputPath, format, ct).ConfigureAwait(false);
		}
		...
		private async Task ExportAuthAsync(IOrganizationServiceAsync2 crm, Guid siteId, string outputPath, string format, CancellationToken ct)
		{
			...
			var results = await crm.RetrieveMultipleAsync(query, ct).ConfigureAwait(false);
