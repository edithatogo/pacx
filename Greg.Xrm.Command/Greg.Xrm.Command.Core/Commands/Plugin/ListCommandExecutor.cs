...
namespace Greg.Xrm.Command.Commands.Plugin
{
	...
	) : ICommandExecutor<ListCommand>
	{
		public async Task<CommandResult> ExecuteAsync(ListCommand command, CancellationToken cancellationToken)
		{
			...
			var crm = await orgServiceRepo.GetCurrentConnectionAsync(cancellationToken).ConfigureAwait(false);
			...
			await Task.WhenAll(packageSearchTask, assemblySearchTask, typeSearchTask, stepSearchTask).ConfigureAwait(false);

			var matchedPackages = await packageSearchTask.ConfigureAwait(false);
			var matchedAssemblies = await assemblySearchTask.ConfigureAwait(false);
			var matchedTypes = await typeSearchTask.ConfigureAwait(false);
			var matchedSteps = await stepSearchTask.ConfigureAwait(false);
			...
			if (matchedPackages.Length > 0)
			{
				var pkgAssemblyResults = await Task.WhenAll(
					matchedPackages.Select(p => assemblyRepo.GetByPackageIdAsync(crm, p.Id, cancellationToken))).ConfigureAwait(false);
			...
			if (!string.IsNullOrWhiteSpace(command.SolutionName))
			{
				...
				var solution = await solutionRepository.GetByUniqueNameAsync(crm, command.SolutionName, cancellationToken).ConfigureAwait(false);
				if (solution == null)
				{
				...
				var solutionAssemblies = await assemblyRepo.GetBySolutionIdAsync(crm, solution.Id, cancellationToken).ConfigureAwait(false);
			...
			{
				var loaded = await assemblyRepo.GetByGuidsAsync(crm, missingIds, cancellationToken).ConfigureAwait(false);
			...
			{
				var types = await typeRepo.GetByAssemblyId(crm, asm.Id, cancellationToken).ConfigureAwait(false);
				var steps = await stepRepo.GetByAssemblyIdAsync(crm, asm.Id, includeInternalStages: false, cancellationToken).ConfigureAwait(false);
			...
			var allImages = await imageRepo.GetByStepIdsAsync(crm, allStepIds).ConfigureAwait(false);
			...
			{
				var loaded = await packageRepo.GetByGuidsAsync(crm, missingPkgIds, cancellationToken).ConfigureAwait(false);
