	...
	) : ICommandExecutor<PushCommand>
	{
		public async Task<CommandResult> ExecuteAsync(PushCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				{
					var result = await ManagePackageRegistrationAsync(command, crm, solution, cancellationToken).ConfigureAwait(false);
					return result;
				}

				if (string.Equals(fileInfo.Extension, ".dll", StringComparison.OrdinalIgnoreCase))
				{
					var result = await ManageAssemblyRegistrationAsync(command, crm, solution, cancellationToken).ConfigureAwait(false);
					return result;
				}
		...
		private async Task<CommandResult> ManagePackageRegistrationAsync(PushCommand command, IOrganizationServiceAsync2 crm, Model.Solution solution, CancellationToken cancellationToken)
		{
			...
			var pluginPackage = await pluginPackageRepository.GetByIdAsync(crm, packageId, cancellationToken).ConfigureAwait(false);
			output.WriteLine("Done", ConsoleColor.Green);
			...
			await pluginPackage.SaveOrUpdateAsync(crm, cancellationToken).ConfigureAwait(false);
			output.WriteLine("Done", ConsoleColor.Green);


			output.Write($"Retrieving assemblies associated with package {packageData.PackageId}...");
			output.WriteLine("Done", ConsoleColor.Green);
			var pluginAssemblyList = await pluginAssemblyRepository.GetByPackageIdAsync(crm, pluginPackage.Id, cancellationToken).ConfigureAwait(false);
		...
		{
			output.Write($"Reading plugin package from {command.Path}...");
			var packageData = await pluginPackageReader.ReadAssemblyFileAsync(command.Path).ConfigureAwait(false);
			if (packageData.HasError)
			{
				output.WriteLine("Failed", ConsoleColor.Red);
			...
			var pluginAssembly = await pluginAssemblyRepository.GetByNameAsync(crm, data.Name, cancellationToken).ConfigureAwait(false);
			output.WriteLine("Done", ConsoleColor.Green);
			...
			try
			{
				await pluginAssembly.SaveOrUpdateAsync(crm, cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
			...
			if (!isNew)
			{
				...
				existingTypes = await pluginTypeRepository.GetByAssemblyId(crm, pluginAssembly.Id, cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
			...
			foreach (var name in packageData.PluginTypes)
			{
				...
				try
				{
					await pluginType.SaveOrUpdateAsync(crm, cancellationToken).ConfigureAwait(false);
					output.WriteLine("Done", ConsoleColor.Green);
				}
				catch (FaultException<OrganizationServiceFault> ex)
				{
