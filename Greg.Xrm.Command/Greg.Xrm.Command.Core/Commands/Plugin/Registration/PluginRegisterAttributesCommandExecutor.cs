		...
		IOrganizationServiceRepository organizationServiceRepository) : ICommandExecutor<PluginRegisterAttributesCommand>
	{
		public async Task<CommandResult> ExecuteAsync(PluginRegisterAttributesCommand command, CancellationToken cancellationToken)
		{
			try
			{
				...
				var publisherId = await EnsurePublisherAsync(crm, command.PublisherUniqueName, command.PublisherName, cancellationToken).ConfigureAwait(false);

				foreach (var asm in assemblies)
				{
					var assemblyId = await RegisterAssemblyAsync(crm, asm, publisherId, command.IsolationMode, cancellationToken).ConfigureAwait(false);

					foreach (var pluginType in asm.PluginTypes)
					{
						var pluginTypeId = await RegisterPluginTypeAsync(crm, pluginType, assemblyId, cancellationToken).ConfigureAwait(false);

						foreach (var step in pluginType.Steps)
						{
							await RegisterStepAsync(crm, step, pluginTypeId, pluginType, cancellationToken).ConfigureAwait(false);
						}

						foreach (var image in pluginType.Images)
						{
							await RegisterImageAsync(crm, image, pluginTypeId, cancellationToken).ConfigureAwait(false);
						}

						foreach (var webhook in pluginType.Webhooks)
						{
							await RegisterWebhookAsync(crm, webhook, pluginTypeId, cancellationToken).ConfigureAwait(false);
						}
					}
				}
		...
		private async Task<Guid> EnsurePublisherAsync(IOrganizationServiceAsync2 crm, string uniqueName, string friendlyName, CancellationToken ct)
		{
			...
			var id = await crm.CreateAsync(publisher, ct).ConfigureAwait(false);
		...
		{
			var dllContent = Convert.ToBase64String(await File.ReadAllBytesAsync(asm.AssemblyPath, ct).ConfigureAwait(false));
			...
			if (result.Entities.Count > 0)
			{
				...
				try
				{
					await crm.UpdateAsync(assembly, ct).ConfigureAwait(false);
				...
				catch (FaultException<Microsoft.Xrm.Sdk.OrganizationServiceFault> ex)
				{
					...
					if (ex.Message.Contains("immutable") || ex.Message.Contains("cannot be changed"))
					{
						...
						await crm.DeleteAsync("pluginassembly", existingId, ct).ConfigureAwait(false);
						assembly.Id = await crm.CreateAsync(assembly, ct).ConfigureAwait(false);
		...
		private async Task<Guid> RegisterPluginTypeAsync(IOrganizationServiceAsync2 crm, PluginTypeMetadata pluginType, Guid assemblyId, CancellationToken ct)
		{
			...
			var id = await crm.CreateAsync(type, ct).ConfigureAwait(false);
		...
		private async Task RegisterStepAsync(IOrganizationServiceAsync2 crm, PluginStepMetadata step, Guid pluginTypeId, PluginTypeMetadata pluginType, CancellationToken ct)
		{
			...
			var msgResult = await crm.RetrieveMultipleAsync(msgQuery, ct).ConfigureAwait(false);
			...
			var filterResult = await crm.RetrieveMultipleAsync(filterQuery, ct).ConfigureAwait(false);
			...
			var existingResult = await crm.RetrieveMultipleAsync(existingQuery, ct).ConfigureAwait(false);

			if (existingResult.Entities.Count > 0)
			{
				stepRecord.Id = existingResult.Entities[0].Id;
				await crm.UpdateAsync(stepRecord, ct).ConfigureAwait(false);
				output.WriteLine($"    Updated step: {stepName}", ConsoleColor.Green);
			}
			else
			{
				await crm.CreateAsync(stepRecord, ct).ConfigureAwait(false);
		...
		private async Task RegisterImageAsync(IOrganizationServiceAsync2 crm, PluginImageMetadata image, Guid pluginTypeId, CancellationToken ct)
		{
			...
			var steps = await crm.RetrieveMultipleAsync(stepQuery, ct).ConfigureAwait(false);
			...
			await crm.CreateAsync(imageRecord, ct).ConfigureAwait(false);
