...
namespace Greg.Xrm.Command.Commands.Plugin.Step
{
	...
	) : ICommandExecutor<ListCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(ListCommand command, CancellationToken cancellationToken)
		{
			...
			var steps = await RetrieveStepsAsync(crm, command, cancellationToken).ConfigureAwait(false);
			...
			var stepInfos = await ProcessStepInformationAsync(crm, steps, showSolutionMembership, cancellationToken).ConfigureAwait(false);
		...
		private async Task<SdkMessageProcessingStep[]?> RetrieveStepsAsync(IOrganizationServiceAsync2 crm, ListCommand command, CancellationToken cancellationToken)
		{
			...
			{
				return await RetrieveStepsByAssemblyAsync(crm, command.AssemblyName, command.ShowInternalPlugins, cancellationToken).ConfigureAwait(false);
			}

			// Filter by plugin type name or ID
			if (!string.IsNullOrWhiteSpace(command.PluginTypeName))
			{
				return await RetrieveStepsByPluginTypeAsync(crm, command.PluginTypeName, command.ShowInternalPlugins, cancellationToken).ConfigureAwait(false);
			}
			...
			if (!string.IsNullOrWhiteSpace(command.TableName))
			{
				...
				var steps = await sdkMessageProcessingStepRepository.GetByTableNameAsync(crm, command.TableName, command.ShowInternalPlugins, cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
				return steps;
			}

			// Default to solution-based retrieval (either specified solution or default solution)
			return await RetrieveStepsBySolutionAsync(crm, command.SolutionName, command.ShowInternalPlugins, cancellationToken).ConfigureAwait(false);
		}
		...
		private async Task<SdkMessageProcessingStep[]?> RetrieveStepsBySolutionAsync(IOrganizationServiceAsync2 crm, string? solutionName, bool includeInternalStages, CancellationToken cancellationToken)
		{
			...
			if (string.IsNullOrWhiteSpace(targetSolutionName))
			{
				...
				targetSolutionName = await organizationServiceRepository.GetCurrentDefaultSolutionAsync().ConfigureAwait(false);
				if (string.IsNullOrWhiteSpace(targetSolutionName))
				{
			...
			var solution = await solutionRepository.GetByUniqueNameAsync(crm, targetSolutionName, cancellationToken).ConfigureAwait(false);
			if (solution == null)
			{
				output.WriteLine("Failed", ConsoleColor.Red);
				return null;
			...
			var steps = await sdkMessageProcessingStepRepository.GetBySolutionAsync(crm, solution.Id, includeInternalStages, cancellationToken).ConfigureAwait(false);
			output.WriteLine("Done", ConsoleColor.Green);
			return steps;
		}
		...
		private async Task<SdkMessageProcessingStep[]?> RetrieveStepsByAssemblyAsync(IOrganizationServiceAsync2 crm, string assemblyName, bool includeInternalStages, CancellationToken cancellationToken)
		{
			...
			if (Guid.TryParse(assemblyName, out var assemblyGuid))
			{
				...
				var steps = await sdkMessageProcessingStepRepository.GetByAssemblyIdAsync(crm, assemblyGuid, includeInternalStages, cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
				return steps;
			}
			else
			{
				...
				var steps = await sdkMessageProcessingStepRepository.GetByAssemblyNameAsync(crm, assemblyName, includeInternalStages, cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
				return steps;
			}
		}
		...
		private async Task<SdkMessageProcessingStep[]?> RetrieveStepsByPluginTypeAsync(IOrganizationServiceAsync2 crm, string pluginTypeName, bool includeInternalStages, CancellationToken cancellationToken)
		{
			...
			if (Guid.TryParse(pluginTypeName, out var pluginTypeGuid))
			{
				...
				var steps = await sdkMessageProcessingStepRepository.GetByPluginTypeIdAsync(crm, pluginTypeGuid, includeInternalStages, cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
				return steps;
			}
			else
			{
				...
				return await RetrieveStepsByPluginTypeNameAsync(crm, pluginTypeName, includeInternalStages, cancellationToken).ConfigureAwait(false);
			}
		}
		...
		private async Task<SdkMessageProcessingStep[]?> RetrieveStepsByPluginTypeNameAsync(IOrganizationServiceAsync2 crm, string pluginTypeName, bool includeInternalStages, CancellationToken cancellationToken)
		{
			...
			var pluginTypes = await pluginTypeRepository.FuzzySearchAsync(crm, pluginTypeName, cancellationToken).ConfigureAwait(false);
			...
			var steps = await sdkMessageProcessingStepRepository.GetByPluginTypeIdAsync(crm, pluginType.Id, includeInternalStages, cancellationToken).ConfigureAwait(false);
			output.WriteLine("Done", ConsoleColor.Green);
			return steps;
		}
		...
		private async Task<List<PluginStepInfo>> ProcessStepInformationAsync(IOrganizationServiceAsync2 crm, SdkMessageProcessingStep[] steps, bool showSolutionMembership, CancellationToken cancellationToken)
		{
			...
			var imagesByStepId = await GetImagesByStepIdsAsync(crm, stepIds).ConfigureAwait(false);
		...
		private async Task<Dictionary<Guid, SdkMessageProcessingStepImage[]>> GetImagesByStepIdsAsync(IOrganizationServiceAsync2 crm, Guid[] stepIds)
		{
			...
			for (int i = 0; i < stepIds.Length; i += batchSize)
			{
				...
				var batchImages = await sdkMessageProcessingStepImageRepository.GetByStepIdsAsync(crm, batch).ConfigureAwait(false);
