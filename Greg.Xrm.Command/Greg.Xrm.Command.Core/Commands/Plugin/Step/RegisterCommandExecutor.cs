...
namespace Greg.Xrm.Command.Commands.Plugin.Step
{
	...
	) : ICommandExecutor<RegisterCommand>
	{
		public async Task<CommandResult> ExecuteAsync(RegisterCommand command, CancellationToken cancellationToken)
		{
			...
			var sdkMessage = await sdkMessageRepository.GetByNameAsync(crm, command.MessageName).ConfigureAwait(false);
			if (sdkMessage == null)
			{
			...
			var pluginTypeList = await pluginTypeRepository.FuzzySearchAsync(crm, command.PluginTypeName, cancellationToken).ConfigureAwait(false);
