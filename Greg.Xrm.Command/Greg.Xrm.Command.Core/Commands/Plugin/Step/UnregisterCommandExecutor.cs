	...
	) : ICommandExecutor<UnregisterCommand>
	{
		public async Task<CommandResult> ExecuteAsync(UnregisterCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				{
					step = await sdkMessageProcessingStepRepository.GetByIdAsync(crm, command.StepId.Value).ConfigureAwait(false);
				}
				else
				{
					var pluginType = await pluginTypeRepository.FuzzySearchAsync(crm, command.PluginTypeName, cancellationToken).ConfigureAwait(false);
					...
					var steps = await sdkMessageProcessingStepRepository.GetByKeyAsync(crm, pluginType[0], command.MessageName, command.PrimaryEntityName, command.Stage!.Value).ConfigureAwait(false);
					if (steps.Length > 1)
					{
						output.WriteLine("Failed", ConsoleColor.Red);
			...
			try
			{
				...
				foreach (var image in images)
				{
					...
					await crm.DeleteAsync(image.EntityName, image.Id, cancellationToken).ConfigureAwait(false);
					output.WriteLine("Done", ConsoleColor.Green);
				}
				...
				if (step.sdkmessageprocessingstepsecureconfigid != null)
				{
					...
					await crm.DeleteAsync(item.LogicalName, item.Id, cancellationToken).ConfigureAwait(false);
					output.WriteLine("Done", ConsoleColor.Green);
				}
			}
			catch (Exception ex)
