	...
	) : ICommandExecutor<UnregisterCommand>
	{
		public async Task<CommandResult> ExecuteAsync(UnregisterCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				{
					pluginType = await pluginTypeRepository.GetByIdAsync(crm, command.TypeId.Value, cancellationToken).ConfigureAwait(false);
				}
				else
				{
					var matches = await pluginTypeRepository.FuzzySearchAsync(crm, command.PluginTypeName!, cancellationToken).ConfigureAwait(false);
					if (matches.Length == 0)
					{
			...
			try
			{
				steps = await sdkMessageProcessingStepRepository.GetByPluginTypeIdAsync(crm, pluginType.Id, false, cancellationToken).ConfigureAwait(false);
			...
			if (steps.Length > 0)
			{
				try
				{
					for (var i = 0; i < steps.Length; i++)
					{
						...
						var images = await sdkMessageProcessingStepImageRepository.GetByStepIdAsync(crm, step.Id).ConfigureAwait(false);

						for (var j = 0; j < images.Length; j++)
						{
							output.Write($"Deleting image {j + 1}/{images.Length} of step {i + 1}/{steps.Length}...");
							await crm.DeleteAsync(images[j].EntityName, images[j].Id, cancellationToken).ConfigureAwait(false);
							output.WriteLine("Done", ConsoleColor.Green);
						}

						output.Write($"Deleting step {i + 1}/{steps.Length} ({step.name})...");
						await crm.DeleteAsync(step.EntityName, step.Id, cancellationToken).ConfigureAwait(false);
						output.WriteLine("Done", ConsoleColor.Green);
						...
						if (step.sdkmessageprocessingstepsecureconfigid != null)
						{
							...
							await crm.DeleteAsync(secureConfig.LogicalName, secureConfig.Id, cancellationToken).ConfigureAwait(false);
							output.WriteLine("Done", ConsoleColor.Green);
						}
					}
				}
			...
			try
			{
				...
				await crm.DeleteAsync(pluginType.EntityName, pluginType.Id, cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
			}
			catch (Exception ex)
			{
