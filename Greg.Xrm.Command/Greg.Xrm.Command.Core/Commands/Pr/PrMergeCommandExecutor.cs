		...
		IOutput output) : ICommandExecutor<PrMergeCommand>
	{
		public async Task<CommandResult> ExecuteAsync(PrMergeCommand command, CancellationToken cancellationToken)
		{
			try
			{
				...
				var pr = await client.PullRequest.Get(owner, name, command.Number).ConfigureAwait(false);
				...
				if (command.WaitForChecks)
				{
					...
					var checksPassed = await WaitForChecksAsync(client, owner, name, pr.Head.Sha, TimeSpan.FromMinutes(30), cancellationToken).ConfigureAwait(false);
					if (!checksPassed)
					{
				...
				{
					PrMergeCommand.MergeMethod.Squash => await client.PullRequest.Merge(owner, name, command.Number, new MergePullRequest { MergeMethod = PullRequestMergeMethod.Squash }).ConfigureAwait(false),
					PrMergeCommand.MergeMethod.Rebase => await client.PullRequest.Merge(owner, name, command.Number, new MergePullRequest { MergeMethod = PullRequestMergeMethod.Rebase }).ConfigureAwait(false),
					_ => await client.PullRequest.Merge(owner, name, command.Number, new MergePullRequest { MergeMethod = PullRequestMergeMethod.Merge }).ConfigureAwait(false),
				};
				...
				if (mergeResult.Merged)
				{
					...
					if (command.DeleteBranch && !string.IsNullOrEmpty(pr.Head?.Ref))
					{
						...
						try
						{
							await client.Git.Reference.Delete(owner, name, $"heads/{pr.Head.Ref}").ConfigureAwait(false);
		...
		private async Task<bool> WaitForChecksAsync(GitHubClient client, string owner, string name, string sha, TimeSpan timeout, CancellationToken ct)
		{
			...
			{
				var checks = await client.Check.Run.GetAllForReference(owner, name, sha).ConfigureAwait(false);
