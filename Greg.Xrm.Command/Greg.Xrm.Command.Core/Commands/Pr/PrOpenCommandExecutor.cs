		...
		IOutput output) : ICommandExecutor<PrOpenCommand>
	{
		public async Task<CommandResult> ExecuteAsync(PrOpenCommand command, CancellationToken cancellationToken)
		{
			try
			{
				...
				{
					return CommandResult.Fail("Unable to determine repository. Specify --repo or configure git remote.");
				}
				...
				if (command.DryRun)
				{
					output.WriteLine("[DRY RUN] Would create pull request:", ConsoleColor.Yellow);
					output.WriteLine($"  PR: (current branch) -> {command.BaseBranch}");
					output.WriteLine($"  Repo: {repo}");
					if (!string.IsNullOrWhiteSpace(command.Title))
					{
						output.WriteLine($"  Title: {command.Title}");
					}
					if (!string.IsNullOrWhiteSpace(command.Body))
					{
						output.WriteLine($"  Body: {command.Body}");
					}
				...
				output.Write($"Creating PR {currentBranch} -> {command.BaseBranch} in {repo}...");
				var newPr = new NewPullRequest(title, currentBranch, command.BaseBranch)
				{
					Body = body,
				};
				var pr = await client.PullRequest.Create(owner, name, newPr).ConfigureAwait(false);
