		...
		IOutput output) : ICommandExecutor<PrTrackCommand>
	{
		public async Task<CommandResult> ExecuteAsync(PrTrackCommand command, CancellationToken cancellationToken)
		{
			try
			{
				...
				if (command.Watch)
				{
					return await WatchPrAsync(client, owner, name, command.Number, command.Format, cancellationToken).ConfigureAwait(false);
				}

				return await DisplayPrStatusAsync(client, owner, name, command.Number, command.Format, cancellationToken).ConfigureAwait(false);
			}
		...
		{
			var pr = await client.PullRequest.Get(owner, name, number).ConfigureAwait(false);

			var checks = await client.Check.Run.GetAllForReference(owner, name, pr.Head.Sha).ConfigureAwait(false);
			var ciStatus = GetCiStatus(checks);

			var reviews = await client.PullRequest.Review.GetAll(owner, name, number).ConfigureAwait(false);
		...
		private async Task<CommandResult> WatchPrAsync(GitHubClient client, string owner, string name, int number, PrTrackCommand.OutputFormat format, CancellationToken ct)
		{
			...
			while (!ct.IsCancellationRequested)
			{
				try
				{
					...
					output.WriteLine($"[{DateTimeOffset.Now:HH:mm:ss}] State: {pr.State} | CI: {ciStatus} | Reviews: {reviewStatus} | Mergeable: {pr.Mergeable?.ToString() ?? "unknown"}");
				...
				{
					output.WriteLine($"[{DateTimeOffset.Now:HH:mm:ss}] Error: {ex.Message}", ConsoleColor.Red);
				}
