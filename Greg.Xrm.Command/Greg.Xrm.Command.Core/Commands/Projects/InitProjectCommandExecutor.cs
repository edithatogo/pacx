...
namespace Greg.Xrm.Command.Commands.Projects
{
	...
	) : ICommandExecutor<InitProjectCommand>
	{
		...
		{
			var connectionSettings = await organizationServiceRepository.GetAllConnectionDefinitionsAsync(cancellationToken).ConfigureAwait(false);
			...
			{
				solutionName = await organizationServiceRepository.GetCurrentDefaultSolutionAsync(cancellationToken).ConfigureAwait(false);
			...
			try
			{
				...
				var crm = await organizationServiceRepository.GetConnectionByNameAsync(authProfileName, cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);

				output.Write($"Retrieving solution {solutionName}...");
				var solution = await solutionRepository.GetByUniqueNameAsync(crm, solutionName, cancellationToken).ConfigureAwait(false);
				if (solution != null)
				{
					output.WriteLine("Done", ConsoleColor.Green);
				}
				...
				await pacxProjectRepository.SaveAsync(project, Environment.CurrentDirectory, cancellationToken).ConfigureAwait(false);
