...
namespace Greg.Xrm.Command.Commands.Relationship
{
	public class CreateNNExplicitStrategy : ICreateNNStrategy
	{
		...
		public async Task<CommandResult> CreateAsync(CreateNNCommand command, string currentSolutionName, int defaultLanguageCode, string publisherPrefix, CancellationToken cancellationToken = default)
		{
			...
			await crm.CheckManyToManyExplicitEligibilityAsync(command.Table1, command.Table2, cancellationToken).ConfigureAwait(false);

			var table1 = await crm.GetEntityMetadataAsync(command.Table1, cancellationToken).ConfigureAwait(false);
			var table2 = await crm.GetEntityMetadataAsync(command.Table2, cancellationToken).ConfigureAwait(false);
			...
			var parentResponse = (ExecuteTransactionResponse)await crm.ExecuteAsync(parentRequest, cancellationToken).ConfigureAwait(false);
			...
			await crm.ExecuteAsync(requestX, cancellationToken).ConfigureAwait(false);
