...
namespace Greg.Xrm.Command.Commands.Relationship
{
	public class CreateNNImplicitStrategy : ICreateNNStrategy
	{
		...
		{
			await crm.CheckManyToManyEligibilityAsync(command.Table1, cancellationToken).ConfigureAwait(false);
			await crm.CheckManyToManyEligibilityAsync(command.Table2, cancellationToken).ConfigureAwait(false);
			...
			var response = (CreateManyToManyResponse)await crm.ExecuteAsync(request, cancellationToken).ConfigureAwait(false);
