...
namespace Greg.Xrm.Command.Commands.Settings
{
	public class CreateCommandExecutor : ICommandExecutor<CreateCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(CreateCommand command, CancellationToken cancellationToken)
		{
			...
			var (publisherPrefix, currentSolutionName, _) = await CheckSolutionAndReturnPublisherPrefixAsync(crm, command.SolutionName, cancellationToken).ConfigureAwait(false);
			...
			try
			{
				...
				await setting.SaveOrUpdateAsync(crm, cancellationToken).ConfigureAwait(false);
				...
				var response = (AddSolutionComponentResponse)await crm.ExecuteAsync(request, cancellationToken).ConfigureAwait(false);
		...
		private async Task<(string?, string?, int?)> CheckSolutionAndReturnPublisherPrefixAsync(IOrganizationServiceAsync2 crm, string? currentSolutionName, CancellationToken cancellationToken)
		{
