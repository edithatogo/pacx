			...
			IAppSettingRepository appSettingRepository) : ICommandExecutor<ExportCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(ExportCommand command, CancellationToken cancellationToken)
		{
			...
			if (command.Origin == Origin.Solution)
			{
				...
				{
					defaultSolutionName = await this.organizationServiceRepository.GetCurrentDefaultSolutionAsync().ConfigureAwait(false);
					if (string.IsNullOrWhiteSpace(defaultSolutionName))
					{
				...
				var solution = await this.solutionRepository.GetByUniqueNameAsync(crm, defaultSolutionName, cancellationToken).ConfigureAwait(false);
				if (solution == null)
				{
			...
			var settingList = await this.settingRepository.GetAllAsync(crm, solutionId, onlyVisible).ConfigureAwait(false);
			...
			var organizationSettingList = await this.organizationSettingRepository.GetByDefinitionsAsync(crm, settingList).ConfigureAwait(false);

			var appSettingList = await this.appSettingRepository.GetByDefinitionsAsync(crm, settingList).ConfigureAwait(false);
			...
			{
				command.OutputFileName = command.OutputFileName.Replace("{version}", DateTimeOffset.Now.ToString("yyyy.MM.dd.HH.mm.ss"));
			}
			...
			{
				return await WriteExcelAsync(command, settingList, organizationSettingList, appSettingList).ConfigureAwait(false);
			}
