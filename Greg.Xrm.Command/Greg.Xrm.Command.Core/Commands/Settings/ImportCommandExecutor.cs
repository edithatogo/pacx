	...
	public class ImportCommandExecutor : ICommandExecutor<ImportCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(ImportCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				importStrategy = await this.importStrategyFactory.CreateAsync(stream, cancellationToken).ConfigureAwait(false);
			...
			try
			{
				actions = await importStrategy.ImportAsync(cancellationToken).ConfigureAwait(false);
				this.output.WriteLine("Done", ConsoleColor.Green);
			}
			catch (Exception ex)
			{
