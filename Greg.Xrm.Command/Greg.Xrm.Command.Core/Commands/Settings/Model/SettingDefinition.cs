...
namespace Greg.Xrm.Command.Commands.Settings.Model
{
	public class SettingDefinition : EntityWrapper
	{
		...
		public class Repository : ISettingDefinitionRepository
		{
			...
			public async Task<IReadOnlyList<SettingDefinition>> GetAllAsync(IOrganizationServiceAsync2 crm, Guid? solutionId, bool onlyVisible, CancellationToken cancellationToken = default)
			{
			...
			public async Task<SettingDefinition?> GetByUniqueNameAsync(IOrganizationServiceAsync2 crm, string uniqueName, CancellationToken cancellationToken = default)
			{
