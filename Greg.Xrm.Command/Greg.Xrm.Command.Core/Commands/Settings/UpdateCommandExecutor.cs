...
namespace Greg.Xrm.Command.Commands.Settings
{
	public class UpdateCommandExecutor : ICommandExecutor<UpdateCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(UpdateCommand command, CancellationToken cancellationToken)
		{
			...
			var settingDefinition = await this.settingDefinitionRepository.GetByUniqueNameAsync(crm, command.Name, cancellationToken).ConfigureAwait(false);
			...
			try
			{
				...
				await settingDefinition.SaveOrUpdateAsync(crm, cancellationToken).ConfigureAwait(false);
