	...
	: ICommandExecutor<ComponentListCommand>
	{
		public async Task<CommandResult> ExecuteAsync(ComponentListCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				solution = await solutionRepository.GetByUniqueNameAsync(crm, currentSolutionName, cancellationToken).ConfigureAwait(false);
				if (solution == null)
				{
			...
			try
			{
				...
				solutionComponents = await solutionComponentRepository.GetBySolutionIdAsync(crm, solution.Id).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
			}
			catch (FaultException<OrganizationServiceFault> ex)
			{
			...
			try
			{
				...
				await componentResolverEngine.ResolveAllAsync(solutionComponents, crm, cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
			}
			catch (Exception ex)
			{
