...
namespace Greg.Xrm.Command.Commands.Solution
{
		...
		) : ICommandExecutor<ComponentMoveCommand>
	{
		public async Task<CommandResult> ExecuteAsync(ComponentMoveCommand command, CancellationToken cancellationToken)
		{
			...
			{
				fromSolutionName = await organizationServiceRepository.GetCurrentDefaultSolutionAsync().ConfigureAwait(false);
			}
			if (string.IsNullOrWhiteSpace(fromSolutionName))
			{
			...
			var fromSolution = await solutionRepository.GetByUniqueNameAsync(crm, fromSolutionName, cancellationToken).ConfigureAwait(false);
			...
			{
				toSolutionName = await organizationServiceRepository.GetCurrentDefaultSolutionAsync().ConfigureAwait(false);
			}
			if (string.IsNullOrWhiteSpace(toSolutionName))
			{
				return CommandResult.Fail("No solution specified and no default solution found.");
			...
			var toSolution = await solutionRepository.GetByUniqueNameAsync(crm, toSolutionName, cancellationToken).ConfigureAwait(false);
			if (toSolution == null)
			{
				output.WriteLine("Failed", ConsoleColor.Red);
			...
			var component = await solutionComponentRepository.GetBySolutionIdAndObjectIdAsync(crm, fromSolution.Id, command.ComponentId).ConfigureAwait(false);
			if (component == null)
			{
				output.WriteLine("Failed", ConsoleColor.Red);
