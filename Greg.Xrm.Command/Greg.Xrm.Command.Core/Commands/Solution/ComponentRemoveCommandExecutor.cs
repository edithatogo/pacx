...
namespace Greg.Xrm.Command.Commands.Solution
{
		...
		) : ICommandExecutor<ComponentRemoveCommand>
	{
		public async Task<CommandResult> ExecuteAsync(ComponentRemoveCommand command, CancellationToken cancellationToken)
		{
			...
			{
				solutionName = await organizationServiceRepository.GetCurrentDefaultSolutionAsync().ConfigureAwait(false);
			}
			if (string.IsNullOrWhiteSpace(solutionName))
			{
			...
			var component = await solutionComponentRepository.GetBySolutionIdAndObjectIdAsync(crm, solution.Id, command.ComponentId).ConfigureAwait(false);
			if (component == null)
			{
				output.WriteLine("Failed", ConsoleColor.Red);
