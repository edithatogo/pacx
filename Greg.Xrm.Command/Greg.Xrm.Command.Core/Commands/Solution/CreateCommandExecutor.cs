	...
	public class CreateCommandExecutor : ICommandExecutor<CreateCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(CreateCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				var exists = (await crm.RetrieveMultipleAsync(query, cancellationToken).ConfigureAwait(false)).Entities.Count > 0;
				...
				var publisherRef = await GetOrCreatePublisherAsync(crm, command, cancellationToken).ConfigureAwait(false);
				...
				{
					await AddApplicationRibbons(solution, crm, cancellationToken).ConfigureAwait(false);
				}
		...
		/// <returns></returns>
		private async Task AddApplicationRibbons(Entity solution, IOrganizationServiceAsync2 crm, CancellationToken cancellationToken)
		{
		...
		private async Task<EntityReference> GetOrCreatePublisherAsync(IOrganizationServiceAsync2 crm, CreateCommand command, CancellationToken cancellationToken)
		{
			...
			publisher.Id = await crm.CreateAsync(publisher, cancellationToken).ConfigureAwait(false);
