		...
		IOrganizationServiceRepository organizationServiceRepository) : ICommandExecutor<GetPublisherListCommand>
	{
		public async Task<CommandResult> ExecuteAsync(GetPublisherListCommand command, CancellationToken cancellationToken)
		{
			try
			{
				...
				string[] blackListPublisher = string.IsNullOrEmpty(command.publisherBlacklist)
					? Array.Empty<string>()
					: command.publisherBlacklist.Split(',', StringSplitOptions.TrimEntries);

				if (command.publisherBlacklist is null)
				{
					blackListPublisher = ["MicrosoftCorporation", "microsoftfirstparty"];
				}
				...
				var listPublisher = (await crm.RetrieveMultipleAsync(query, cancellationToken).ConfigureAwait(false)).Entities;
