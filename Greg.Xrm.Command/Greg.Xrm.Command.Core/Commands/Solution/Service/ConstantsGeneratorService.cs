...
namespace Greg.Xrm.Command.Commands.Solution.Service
{
	public class ConstantsGeneratorService : IConstantsGeneratorService
	{
			...
			CancellationToken cancellationToken)
		{
			...
			var entitiesResponse = (RetrieveAllEntitiesResponse)await crm.ExecuteAsync(new RetrieveAllEntitiesRequest
			{
				EntityFilters = EntityFilters.Entity | EntityFilters.Attributes | EntityFilters.Relationships
			}, cancellationToken).ConfigureAwait(false);
			var entitiesMetadata = entitiesResponse.EntityMetadata;

			// 2. Retrieve all global option sets
			output.WriteLine("  Reading global option sets...", ConsoleColor.Gray);
			var optionSetsResponse = (RetrieveAllOptionSetsResponse)await crm.ExecuteAsync(new RetrieveAllOptionSetsRequest(), cancellationToken).ConfigureAwait(false);
			...
			var solutionEntityGuids = await GetEntityGuidsFromSolutionAsync(crm, request.SolutionName, cancellationToken).ConfigureAwait(false);
		...
		private static async Task<HashSet<Guid>> GetEntityGuidsFromSolutionAsync(IOrganizationServiceAsync2 crm, string solutionName, CancellationToken cancellationToken = default)
		{
			var solutionQuery = new QueryExpression("solution");
			solutionQuery.Criteria.AddCondition(new ConditionExpression("uniquename", ConditionOperator.Equal, solutionName));
			var solutions = await crm.RetrieveMultipleAsync(solutionQuery, cancellationToken).ConfigureAwait(false);
			...
			var components = await crm.RetrieveMultipleAsync(componentQuery, cancellationToken).ConfigureAwait(false);
