		...
		IOrganizationServiceRepository organizationServiceRepository) : ICommandExecutor<SolutionComponentMoveCommand>
	{
		public async Task<CommandResult> ExecuteAsync(SolutionComponentMoveCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				var sourceSolution = await FindSolutionAsync(crm, command.FromSolution, cancellationToken).ConfigureAwait(false);
				...
				var targetSolution = await FindSolutionAsync(crm, command.ToSolution, cancellationToken).ConfigureAwait(false);
				...
				var component = await FindComponentAsync(crm, command.ComponentName, componentTypeCode, sourceSolution.Id, cancellationToken).ConfigureAwait(false);
				if (component == null)
				{
				...
				await crm.ExecuteAsync(removeRequest, cancellationToken).ConfigureAwait(false);
				output.WriteLine(" Done", ConsoleColor.Green);
