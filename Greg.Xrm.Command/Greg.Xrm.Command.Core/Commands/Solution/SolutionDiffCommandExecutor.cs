...
namespace Greg.Xrm.Command.Commands.Solution
{
		...
		IOrganizationServiceRepository organizationServiceRepository) : ICommandExecutor<SolutionDiffCommand>
	{
		public async Task<CommandResult> ExecuteAsync(SolutionDiffCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				var sourceComponents = await GetSolutionComponentsAsync(crm, command.Source, command.ComponentType, cancellationToken).ConfigureAwait(false);
				var targetComponents = await GetSolutionComponentsAsync(crm, command.Target, command.ComponentType, cancellationToken).ConfigureAwait(false);
			...
			IOrganizationServiceAsync2 crm, string solutionName, string? componentTypeFilter, CancellationToken ct)
		{
			...
			var solutionResult = await crm.RetrieveMultipleAsync(solutionQuery, ct).ConfigureAwait(false);
			...
			var componentResult = await crm.RetrieveMultipleAsync(componentQuery, ct).ConfigureAwait(false);
			...
			foreach (var comp in componentResult.Entities)
			{
				...
				var componentName = await ResolveComponentNameAsync(crm, componentType, objectId, ct).ConfigureAwait(false);
		...
		private sealed class SolutionChange
		{
