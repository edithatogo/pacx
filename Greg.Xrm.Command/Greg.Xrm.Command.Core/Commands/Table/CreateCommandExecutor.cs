...
namespace Greg.Xrm.Command.Commands.Table
{
	public class CreateCommandExecutor : ICommandExecutor<CreateCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(CreateCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				var defaultLanguageCode = await crm.GetDefaultLanguageCodeAsync(cancellationToken).ConfigureAwait(false);

				var currentSolutionName = command.SolutionName;
				if (string.IsNullOrWhiteSpace(currentSolutionName))
				{
					currentSolutionName = await organizationServiceRepository.GetCurrentDefaultSolutionAsync(cancellationToken).ConfigureAwait(false);
				...
				var solutionList = (await crm.RetrieveMultipleAsync(query, cancellationToken).ConfigureAwait(false)).Entities.ToList();
				...
				var entityMetadata = new EntityMetadata
				{
					...
					DisplayCollectionName = await SetTableDisplayCollectionNameAsync(command, defaultLanguageCode, cancellationToken),
				...
				var response = (CreateEntityResponse)await crm.ExecuteAsync(request, cancellationToken).ConfigureAwait(false);
				output.WriteLine(" Done", ConsoleColor.Green);
				...
				await crm.ExecuteAsync(request1, cancellationToken).ConfigureAwait(false);
				output.WriteLine(" Done", ConsoleColor.Green);
		...
		private async Task<Label> SetTableDisplayCollectionNameAsync(CreateCommand command, int defaultLanguageCode, CancellationToken cancellationToken)
		{
			...
			var pluralName = await pluralizer.GetPluralForAsync(command.DisplayName ?? string.Empty, cancellationToken).ConfigureAwait(false);
