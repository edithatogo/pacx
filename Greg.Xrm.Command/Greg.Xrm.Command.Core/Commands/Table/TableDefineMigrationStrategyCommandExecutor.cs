...
namespace Greg.Xrm.Command.Commands.Table
{
	public class TableDefineMigrationStrategyCommandExecutor : ICommandExecutor<TableDefineMigrationStrategyCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(TableDefineMigrationStrategyCommand command, CancellationToken cancellationToken)
		{
			...
			var tableIds = (await crm.RetrieveMultipleAsync(query, cancellationToken).ConfigureAwait(false))
				.Entities
			...
			var tableNames = (await crm.RetrieveMultipleAsync(query, cancellationToken).ConfigureAwait(false))
				.Entities
			...
			var (missingTables, graph) = await this.tableGraphBuilder.BuildGraphAsync(crm, tableNames, command.IncludeSecurityTables, command.SkipMissingTables).ConfigureAwait(false);
