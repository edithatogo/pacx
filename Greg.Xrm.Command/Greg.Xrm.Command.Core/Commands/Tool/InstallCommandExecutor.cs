...
namespace Greg.Xrm.Command.Commands.Tool
{
			...
			IStorage storage) : ICommandExecutor<InstallCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(InstallCommand command, CancellationToken cancellationToken)
		{
			...
			{
				return await DownloadPluginFromNuget(command, cancellationToken).ConfigureAwait(false);
			}
			else
			{
				return await DownloadPluginFromFile(command, cancellationToken).ConfigureAwait(false);
			}
		}
		...
		private async Task<(bool, PackageIdentity? packageIdentity, bool)> ValidatePackage(SourceRepository repository, string packageId, string packageVersion, SourceCacheContext cacheContext, CancellationToken cancellationToken)
		{
			...
			var packageMetadataResource = await repository.GetResourceAsync<PackageMetadataResource>().ConfigureAwait(false);

			var packageMetadata = (await packageMetadataResource.GetMetadataAsync(packageId, false, true, cacheContext, logger, cancellationToken).ConfigureAwait(false))?.ToList() ?? [];
		...
		private async Task<bool> ExtractPackageStream(Stream? packageStream, string packageId, string path, CancellationToken cancellationToken)
		{
			...
			using (var packageReader = new PackageArchiveReader(packageStream))
			{
				...
				var fwks = await packageReader.GetSupportedFrameworksAsync(cancellationToken).ConfigureAwait(false);
				...
				var validFiles = await ExtractValidFilesFromPackage(packageReader, bestSupportedFramework, cancellationToken).ConfigureAwait(false);
		...
		private async Task<bool> DownloadPackage(PackageIdentity targetPackage, string path, SourceRepository repository, SourceCacheContext cacheContext, CancellationToken cancellationToken)
		{
			try
			{
				...
				return await ExtractPackageStream(packageStream, targetPackage.Id, path, cancellationToken).ConfigureAwait(false);
		...
		private async Task<(bool, PackageIdentity?)> GetCorePackageIdentity(SourceRepository repository, SourceCacheContext cacheContext, CancellationToken cancellationToken)
		{
			...
			var packageMetadata = (await packageMetadataResource.GetMetadataAsync(corePackageId, false, true, cacheContext, logger, cancellationToken).ConfigureAwait(false))?.ToList() ?? [];
			if (packageMetadata.Count == 0)
			{
		...
		private async Task<CommandResult> DownloadPluginFromNuget(InstallCommand command, CancellationToken cancellationToken)
		{
			...
			var (isValid, targetPackage, hasOtherDependencies) = await ValidatePackage(repository, command.Name, command.Version, cacheContext, cancellationToken).ConfigureAwait(false);
			...
			{
				var dependencyInfoResource = await repository.GetResourceAsync<DependencyInfoResource>().ConfigureAwait(false);
				var (foundCore, corePackage) = await GetCorePackageIdentity(repository, cacheContext, cancellationToken).ConfigureAwait(false);
				...
				packagesToInstall = await DependencyUtility.GetDeltaForPackage(
				   ...
				   NuGetFramework.Parse(AppContext.TargetFrameworkName!), cacheContext, logger, cancellationToken).ConfigureAwait(false);
			...
			{
				result &= await DownloadPackage(new PackageIdentity(currentPackage.Key, currentPackage.Value), targetPackage.Id, repository, new SourceCacheContext(), CancellationToken.None).ConfigureAwait(false);
			}
		...
		private async Task<CommandResult> DownloadPluginFromFile(InstallCommand command, CancellationToken cancellationToken)
		{
			...
			await packageStream.CopyToAsync(infoStream, cancellationToken).ConfigureAwait(false);
			...
			bool extractResult = await ExtractPackageStream(packageStream, packageId, packageId, cancellationToken).ConfigureAwait(false);
			if (!extractResult)
			{
