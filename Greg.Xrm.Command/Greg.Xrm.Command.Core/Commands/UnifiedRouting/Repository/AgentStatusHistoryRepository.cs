...
namespace Greg.Xrm.Command.Commands.UnifiedRouting.Repository
{
	public class AgentStatusHistoryRepository
	{
		...
		public async Task<DataCollection<Entity>> GetAgentStatusHistoryByQueue(string queueName, DateTime timeQuery, CancellationToken cancellationToken = default)
		{
		...
		public async Task<Entity?> GetAgentStatusHistoryByAgentMail(string agentEmail, DateTime timeQuery, CancellationToken cancellationToken = default)
		{
			var query = new QueryExpression(nameof(msdyn_agentcapacityupdatehistory));
			query.NoLock = true;
			// Add columns to query.ColumnSet
