		...
		public async Task<CommandResult> ExecuteAsync(SetCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				if (languagePairs.Count > 0)
				{
					...
					var langResponse = (RetrieveAvailableLanguagesResponse)await crm.ExecuteAsync(new RetrieveAvailableLanguagesRequest(), cancellationToken).ConfigureAwait(false);
				...
				await crm.UpdateAsync(userSettings, cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
