...
namespace Greg.Xrm.Command.Commands.Views
{
	...
	: ICommandExecutor<CloneCommand>
	{
		public async Task<CommandResult> ExecuteAsync(CloneCommand command, CancellationToken cancellationToken)
		{
			...
			var (result, view) = await viewRetriever.GetByNameAsync(crm, command.QueryType, command.OldName, command.TableName).ConfigureAwait(false);
			...
			try
			{
				...
				await newView.SaveOrUpdateAsync(crm, cancellationToken).ConfigureAwait(false);
