	...
	) : IViewRetrieverService
	{
			...
			string? tableName)
		{
			...
			if (queryType == QueryType1.SavedQuery)
			{
				...
				var savedQueries = await savedQueryRepository.GetByNameAsync(crm, viewName).ConfigureAwait(false);
			...
			if (queryType == QueryType1.UserQuery)
			{
				...
				var userQueries = await userQueryRepository.GetByNameAsync(crm, viewName).ConfigureAwait(false);
