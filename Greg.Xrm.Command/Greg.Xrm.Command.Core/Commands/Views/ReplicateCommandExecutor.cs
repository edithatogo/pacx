...
namespace Greg.Xrm.Command.Commands.Views
{
	...
	) : ICommandExecutor<ReplicateCommand>
	{
		public async Task<CommandResult> ExecuteAsync(ReplicateCommand command, CancellationToken cancellationToken)
		{
			...
			var (result, view) = await viewRetriever.GetByNameAsync(crm, QueryType1.SavedQuery, command.ViewName, command.TableName).ConfigureAwait(false);
			if (view == null) return result;

			output.Write($"Retrieving other views from table '{view.returnedtypecode}'...");
			var otherViews = await savedQueryRepository.GetByTableNameAsync(crm, view.returnedtypecode).ConfigureAwait(false);
			output.WriteLine("Done", ConsoleColor.Green);
			...
			try
			{
				...
				var operationResult = await Replicator.PropagateLayoutAsync(crm, (SavedQuery)view, otherViews, true, true, true).ConfigureAwait(false);
			...
			try
			{
				...
				await crm.ExecuteAsync(publishXml, cancellationToken).ConfigureAwait(false);
