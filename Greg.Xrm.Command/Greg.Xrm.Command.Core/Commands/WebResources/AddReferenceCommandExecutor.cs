...
namespace Greg.Xrm.Command.Commands.WebResources
{
		...
		: ICommandExecutor<AddReferenceCommand>
	{
		public async Task<CommandResult> ExecuteAsync(AddReferenceCommand command, CancellationToken cancellationToken)
		{
			...
			var (succeeded, projectFile) = await webResourceProjectFileRepository.TryReadAsync(root).ConfigureAwait(false);
			if (!succeeded)
			{
			...
			await webResourceProjectFileRepository.SaveAsync(root, solution.PublisherCustomizationPrefix!, projectFile).ConfigureAwait(false);
