...
namespace Greg.Xrm.Command.Commands.WebResources
{
	public class ApplyIconsCommandExecutor : ICommandExecutor<ApplyIconsCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(ApplyIconsCommand command, CancellationToken cancellationToken)
		{
			...
			{
				tableSolutionName = await organizationServiceRepository.GetCurrentDefaultSolutionAsync().ConfigureAwait(false);
				if (string.IsNullOrWhiteSpace(tableSolutionName))
				{
			...
			{
				webResourceSolutionName = await organizationServiceRepository.GetCurrentDefaultSolutionAsync().ConfigureAwait(false);
				if (string.IsNullOrWhiteSpace(webResourceSolutionName))
				{
			...
			var tableSolution = await this.solutionRepository.GetByUniqueNameAsync(crm, tableSolutionName, cancellationToken).ConfigureAwait(false);
			...
			{
				webResourceSolution = await this.solutionRepository.GetByUniqueNameAsync(crm, webResourceSolutionName, cancellationToken).ConfigureAwait(false);
			}
			...
			var tables = await GetTableNamesBySolution(crm, tableSolution, cancellationToken).ConfigureAwait(false);
			if (tables.Count == 0)
			{
			...
			var images = await GetImagesBySolution(crm, webResourceSolution, cancellationToken).ConfigureAwait(false);
			if (images.Count == 0)
			{
			...
			if (successCount > 0)
			{
				publishSucceeded = await PublishAllCustomizationsAsync(crm, xml, cancellationToken).ConfigureAwait(false);
			}
		...
		private async Task<bool> PublishAllCustomizationsAsync(IOrganizationServiceAsync2 crm, StringBuilder xml, CancellationToken cancellationToken)
		{
			try
			{
		...
		private async Task<IReadOnlyList<EntityMetadata>> GetTableNamesBySolution(IOrganizationServiceAsync2 crm, Model.Solution solution, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				var response = (RetrieveMetadataChangesResponse)await crm.ExecuteAsync(new RetrieveMetadataChangesRequest { Query = query2 }, cancellationToken).ConfigureAwait(false);
		...
		private async Task<IReadOnlyCollection<string>> GetImagesBySolution(IOrganizationServiceAsync2 crm, Model.Solution solution, CancellationToken cancellationToken)
		{
			try
			{
				var webResources = await this.webResourceRepository.GetBySolutionAsync(crm, solution.uniquename, false, cancellationToken).ConfigureAwait(false);
