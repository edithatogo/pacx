...
namespace Greg.Xrm.Command.Commands.WebResources
{
	public class InitCommandExecutor : ICommandExecutor<InitCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(InitCommand command, CancellationToken cancellationToken)
		{
			...
			{
				return await InitRemoteAsync(command, cancellationToken).ConfigureAwait(false);
			}

			return await InitLocalAsync(command, cancellationToken).ConfigureAwait(false);
		}



		private async Task<CommandResult> InitLocalAsync(InitCommand command, CancellationToken cancellationToken)
		{
			...
			var (success, publisherPrefix) = await TryCheckSolution(crm, currentSolutionName, cancellationToken).ConfigureAwait(false);
			if (!success)
			{
			...
			try
			{
				await this.webResourceProjectFileRepository.SaveAsync(folder, publisherPrefix).ConfigureAwait(false);
		...
		private async Task<CommandResult> InitRemoteAsync(InitCommand command, CancellationToken cancellationToken)
		{
			...
			var webResourceList = await this.webResourceRepository.GetBySolutionAsync(crm, currentSolutionName, true, cancellationToken).ConfigureAwait(false);
			...
			foreach (var webResource in webResourceList)
			{
				...
				try
				{
					var content = await webResource.GetContentAsync(crm, cancellationToken).ConfigureAwait(false);
		...
		private async Task<(bool, string)> TryCheckSolution(IOrganizationServiceAsync2 crm, string currentSolutionName, CancellationToken cancellationToken)
		{
			output.WriteLine("Checking solution existence and retrieving publisher");

			var solution = await this.solutionRepository.GetByUniqueNameAsync(crm, currentSolutionName, cancellationToken).ConfigureAwait(false);
			if (solution == null)
			{
