	...
	public class JsCreateCommandExecutor : ICommandExecutor<JsCreateCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(JsCreateCommand command, CancellationToken cancellationToken)
		{
			...
			if (root == null)
			{
				...
				return await CreateFileAsync(command, crm, directory, cancellationToken).ConfigureAwait(false);
			}
			...
			return await CreateFileAsync(command, crm, scriptsFolder, cancellationToken).ConfigureAwait(false);
		}
		...
		private async Task<CommandResult> CreateFileAsync(JsCreateCommand command, IOrganizationServiceAsync2 crm, DirectoryInfo directory, CancellationToken cancellationToken)
		{
			...
			var (success, uniqueName) = await TryCheckSolution(crm, currentSolutionName, cancellationToken).ConfigureAwait(false);
				if (!success)
				{
			...
			var template = await this.jsTemplateManager.GetTemplateForAsync(command.Type, string.IsNullOrWhiteSpace(command.TableName)).ConfigureAwait(false);
			...
			await File.WriteAllTextAsync(fileName, fileContent, cancellationToken).ConfigureAwait(false);
