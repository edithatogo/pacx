	...
	public class JsSetTemplateCommandExecutor : ICommandExecutor<JsSetTemplateCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(JsSetTemplateCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				templateContent = await File.ReadAllTextAsync(command.FileName, cancellationToken).ConfigureAwait(false);
			...
			await this.jsTemplateManager.SetTemplateForAsync(command.Type, global, templateContent).ConfigureAwait(false);
			this.output.WriteLine("Done", ConsoleColor.Green);
