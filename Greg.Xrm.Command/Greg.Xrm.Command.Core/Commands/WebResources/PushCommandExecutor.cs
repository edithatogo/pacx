...
namespace Greg.Xrm.Command.Commands.WebResources
{
			...
			: ICommandExecutor<PushCommand>
	{
		public async Task<CommandResult> ExecuteAsync(PushCommand command, CancellationToken cancellationToken)
		{
			...
			var solution = await solutionRepository.GetByUniqueNameAsync(crm, currentSolutionName, cancellationToken).ConfigureAwait(false);
			output.WriteLine("Done", ConsoleColor.Green);
			if (solution == null)
			{
			...
			try
			{
				...
				if (command.Reference != null)
				{
					...
					var (result, projectFile) = await webResourceProjectFileRepository.TryReadAsync(rootDirectory).ConfigureAwait(false);
					if (!result)
					{
			...
			try
			{
				...
				currentWebResources = await webResourceRepository.GetByNameAsync(crm, fileNames, true).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
			}
			catch (Exception ex)
			{
			...
			await UpdateRemoteFromLocalAsync(crm, files, currentWebResources, command.NoAction, cancellationToken).ConfigureAwait(false);
			...
			{
				var result = await PublishUpdatedWebResourcesAsync(command, crm, publishXmlRequest, cancellationToken).ConfigureAwait(false);
				if (!result.IsSuccess) return result;
			}



			var result1 = await AddWebResourcesToSolutionAsync(crm, solution, currentWebResources, command.Verbose, cancellationToken).ConfigureAwait(false);
			...
			CancellationToken cancellationToken)
		{
			...
			foreach (var file in files)
			{
				...
				try
				{
					var fileContent = await File.ReadAllBytesAsync(file.Source).ConfigureAwait(false);
					...
					if (webResource == null)
					{
						...
						if (!noAction)
						{
							await webResource.SaveOrUpdateAsync(crm, cancellationToken).ConfigureAwait(false);
		...
		private async Task<CommandResult> PublishUpdatedWebResourcesAsync(PushCommand command, IOrganizationServiceAsync2 crm, PublishXmlRequest request, CancellationToken cancellationToken)
		{
		...
		private async Task<CommandResult> AddWebResourcesToSolutionAsync(IOrganizationServiceAsync2 crm, Model.Solution solution, List<WebResource> currentWebResources, bool verbose, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				var result = await solution.UpsertSolutionComponentsAsync(crm, currentWebResources, ComponentType.WebResource, cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
