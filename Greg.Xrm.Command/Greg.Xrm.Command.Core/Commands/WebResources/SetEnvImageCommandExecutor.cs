...
namespace Greg.Xrm.Command.Commands.WebResources
{
		...
		IWebResourceRepository webResourceRepository) : ICommandExecutor<SetEnvImageCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(SetEnvImageCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				var webResourceList = await webResourceRepository.GetByNameAsync(crm, new[] { command.WebResourceUniqueName }, false).ConfigureAwait(false);
			...
			try
			{
				...
				{
					theme.Id = await crm.CreateAsync(theme, cancellationToken).ConfigureAwait(false);
				}
				else
				{
					await crm.UpdateAsync(theme, cancellationToken).ConfigureAwait(false);
				}
				output.WriteLine("Done", ConsoleColor.Green);
			}
			catch (FaultException<OrganizationServiceFault> ex)
