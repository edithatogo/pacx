		...
		public async Task<CommandResult> ExecuteAsync(WebResourceMapCommand command, CancellationToken cancellationToken)
		{
			try
			{
				...
				foreach (var mapping in config.Mappings)
				{
					...
					var result = await UploadWebResourceAsync(crm, mapping, solutionId, command.Force, cancellationToken).ConfigureAwait(false);
				...
				if (command.Publish && (uploaded + updated) > 0)
				{
					...
					await crm.ExecuteAsync(publishRequest, cancellationToken).ConfigureAwait(false);
					output.WriteLine(" Done", ConsoleColor.Green);
				}
