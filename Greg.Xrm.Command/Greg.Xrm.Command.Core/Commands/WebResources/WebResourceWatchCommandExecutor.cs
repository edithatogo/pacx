		...
		ISolutionRepository solutionRepository) : ICommandExecutor<WebResourceWatchCommand>
	{
		...
		public async Task<CommandResult> ExecuteAsync(WebResourceWatchCommand command, CancellationToken cancellationToken)
		{
			try
			{
				...
				{
					var solution = await solutionRepository.GetByUniqueNameAsync(crm, command.SolutionUniqueName, cancellationToken).ConfigureAwait(false);
					if (solution == null)
					{
				...
				foreach (var mapping in config.Mappings)
				{
					...
					await UploadWebResourceAsync(crm, mapping, solutionId, true, cancellationToken).ConfigureAwait(false);
				}
				...
				if (command.Poll)
				{
					await WatchWithPollingAsync(crm, config, solutionId, command.PollIntervalMs, command.Publish, cancellationToken).ConfigureAwait(false);
				}
				else
				{
					await WatchWithFileSystemWatcherAsync(crm, config, solutionId, command.DebounceMs, command.Publish, cancellationToken).ConfigureAwait(false);
				}
		...
		private void OnFileChanged(string filePath, List<WebResourceMapping> mappings, CancellationToken ct)
		{
			...
			output.WriteLine($"[{DateTimeOffset.Now:HH:mm:ss}] Detected change: {Path.GetFileName(filePath)}", ConsoleColor.DarkCyan);
		}
			...
			while (!ct.IsCancellationRequested)
			{
				foreach (var mapping in config.Mappings)
				{
					...
					if (!lastModified.TryGetValue(fullPath, out var previous) || lastWrite > previous)
					{
						...
						output.WriteLine($"[{DateTimeOffset.Now:HH:mm:ss}] Detected change: {Path.GetFileName(fullPath)}", ConsoleColor.DarkCyan);
						await UploadWebResourceAsync(crm, mapping, solutionId, true, ct).ConfigureAwait(false);

						if (publish)
						{
							await PublishAsync(crm, ct).ConfigureAwait(false);
						}
					}
				}
		...
		private async Task PublishAsync(IOrganizationServiceAsync2 crm, CancellationToken ct)
		{
			...
			await crm.ExecuteAsync(publishRequest, ct).ConfigureAwait(false);
			output.WriteLine(" Done", ConsoleColor.Green);
		}
		...
		{
			var content = await File.ReadAllBytesAsync(mapping.LocalPath, ct).ConfigureAwait(false);
			...
			if (result.Entities.Count > 0)
			{
				...
				wr.Id = existingId;
				await crm.UpdateAsync(wr, ct).ConfigureAwait(false);
				output.WriteLine($"  UPDATED: {mapping.UniqueName} <- {mapping.LocalPath}", ConsoleColor.Green);
			}
			else
			{
				wr.Id = await crm.CreateAsync(wr, ct).ConfigureAwait(false);
