	...
	: ICommandExecutor<ListCommand>
	{
		public async Task<CommandResult> ExecuteAsync(ListCommand command, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				workflowList = await workflowRepository.SearchByNameAndSolutionAndCategoryAsync(crm, command.SearchQuery.Trim(), solutionName, command.Category, cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
			}
			catch (Exception ex)
			{
