...
#pragma warning restore IDE1006 // Naming Styles
		...
		public class Repository : IDependencyRepository
		{
			...
			public async Task<DependencyList> GetDependenciesAsync(IOrganizationServiceAsync2 crm, ComponentType componentType, Guid componentId, bool? forDelete = false, CancellationToken cancellationToken = default)
			{
				...
				if (forDelete.GetValueOrDefault())
				{
					...
					var response = (RetrieveDependenciesForDeleteResponse)await crm.ExecuteAsync(request, cancellationToken).ConfigureAwait(false);
				...
				else
				{
					...
					var response = (RetrieveDependentComponentsResponse)await crm.ExecuteAsync(request, cancellationToken).ConfigureAwait(false);
					entities = response.EntityCollection;
				}
				...
				await ResolveDependencyNamesAsync(dependencies, resolverFactory, cancellationToken).ConfigureAwait(false);

				return new DependencyList(dependencies);
			}

			private static async Task ResolveDependencyNamesAsync(Dependency[] dependencies, ComponentResolverFactory resolverFactory, CancellationToken cancellationToken)
			{
				...
				foreach (var dependencyGroup in dependencyGroups)
				{
					...
					if (resolver != null)
					{
						...
						var componentNames = await resolver.GetNamesAsync(componentIds, cancellationToken).ConfigureAwait(false);
