...
namespace Greg.Xrm.Command.Model
{
	public class SavedQuery : TableView
	{
		...
		public override async Task<bool> SaveOrUpdateAsync(IOrganizationServiceAsync2 service, CancellationToken cancellationToken = default)
		{
			...
			if (IsNew) // isNew
			{
				target.Id = await service.CreateAsync(target, cancellationToken).ConfigureAwait(false);
				preImage.Id = target.Id;
			}
			else if (target.Attributes.Count > 0)
			{
				var postImage = me.GetPostImage();
				await service.UpdateAsync(postImage, cancellationToken).ConfigureAwait(false);
			}
