...
namespace Greg.Xrm.Command.Model
{
	public class Solution : EntityWrapper
	{
		...
		public async Task<UpsertSolutionComponentResult<Entity>> UpsertSolutionComponentsAsync(IOrganizationServiceAsync2 crm, IReadOnlyCollection<Entity> componentList, ComponentType componentType, CancellationToken cancellationToken = default)
		{
			return await UpsertSolutionComponentsAsync(crm, componentList, x => x.Id, componentType, cancellationToken).ConfigureAwait(false);
		}

		public async Task<UpsertSolutionComponentResult<EntityWrapper>> UpsertSolutionComponentsAsync(IOrganizationServiceAsync2 crm, IReadOnlyCollection<EntityWrapper> componentList, ComponentType componentType, CancellationToken cancellationToken = default)
		{
			return await UpsertSolutionComponentsAsync(crm, componentList, x => x.Id, componentType, cancellationToken).ConfigureAwait(false);
		}


		private async Task<UpsertSolutionComponentResult<T>> UpsertSolutionComponentsAsync<T>(IOrganizationServiceAsync2 crm, IReadOnlyCollection<T> componentList, Func<T, Guid> idAccessor, ComponentType componentType, CancellationToken cancellationToken)
		{
			...
			var existingComponents = await crm.RetrieveMultipleAsync(query, cancellationToken).ConfigureAwait(false);
			...
			var response = (ExecuteMultipleResponse)await crm.ExecuteAsync(executeMultipleRequest, cancellationToken).ConfigureAwait(false);
...
#pragma warning restore CS8604 // Possible null reference argument.
		...
		public class Repository : ISolutionRepository
		{
			...
			public async Task<ITemporarySolution> CreateTemporarySolutionAsync(IOrganizationServiceAsync2 crm, EntityReference publisherRef, CancellationToken cancellationToken = default)
			{
				var solutionName = $"pacx_temp_{DateTimeOffset.Now.Ticks}";
				...
				solution.Id = await crm.CreateAsync(solution, cancellationToken).ConfigureAwait(false);

				return new TemporarySolution(crm, this.output, new Solution(solution));
			}

			public async Task<Solution?> GetByUniqueNameAsync(IOrganizationServiceAsync2 crm, string uniqueName, CancellationToken cancellationToken = default)
			{
