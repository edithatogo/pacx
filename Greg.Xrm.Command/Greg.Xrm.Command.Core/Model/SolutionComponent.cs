...
#pragma warning restore IDE1006 // Naming Styles
		...
		{
			public async Task<SolutionComponent?> GetBySolutionIdAndObjectIdAsync(IOrganizationServiceAsync2 crm, Guid solutionId, Guid objectId, CancellationToken cancellationToken = default)
			{
			...
			public async Task<List<SolutionComponent>> GetBySolutionIdAsync(IOrganizationServiceAsync2 crm, Guid solutionId, CancellationToken cancellationToken = default)
			{
				var query = new QueryExpression("solutioncomponent");
				query.ColumnSet.AddColumns("componenttype", "rootsolutioncomponentid", "solutionid", "objectid", "ismetadata");
				query.Criteria.AddCondition("solutionid", ConditionOperator.Equal, solutionId);
				...
				do
				{
					result = await crm.RetrieveMultipleAsync(query, cancellationToken).ConfigureAwait(false);
