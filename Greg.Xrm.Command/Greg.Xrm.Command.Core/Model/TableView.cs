...
namespace Greg.Xrm.Command.Model
{
	public abstract class TableView : EntityWrapper
	{
		...
		public abstract class Repository<T> where T : TableView
		{
			...
			public async Task<IReadOnlyList<T>> GetByIdAsync(IOrganizationServiceAsync2 crm, IEnumerable<Guid> ids, CancellationToken cancellationToken = default)
			{
			...
			public async Task<IReadOnlyList<T>> GetContainingAsync(IOrganizationServiceAsync2 crm, string tableName, string columnName, CancellationToken cancellationToken = default)
			{
				var query = new QueryExpression(this.entityName);
				query.ColumnSet.AddColumns(nameof(name), nameof(querytype), nameof(fetchxml), nameof(layoutxml), nameof(returnedtypecode));
				if (otherColumns.Length > 0)
			...
			public async Task<IReadOnlyList<T>> GetByTableNameAsync(IOrganizationServiceAsync2 crm, string tableName, CancellationToken cancellationToken = default)
			{
				var query = new QueryExpression(this.entityName);
				query.ColumnSet.AddColumns(nameof(name), nameof(querytype), nameof(fetchxml), nameof(layoutxml), nameof(returnedtypecode));
				if (otherColumns.Length > 0)
			...
			public async Task<IReadOnlyList<T>> GetByNameAsync(IOrganizationServiceAsync2 crm, string viewName, CancellationToken cancellationToken = default)
			{
				var query = new QueryExpression(this.entityName);
				query.ColumnSet.AddColumns(nameof(name), nameof(querytype), nameof(fetchxml), nameof(layoutxml), nameof(returnedtypecode));
				if (otherColumns.Length > 0)
