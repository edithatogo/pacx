...
namespace Greg.Xrm.Command.Model
{
	public class TemporarySolution : ITemporarySolution
	{
		...
		public async Task AddComponentAsync(Guid componentId, ComponentType componentType, CancellationToken cancellationToken = default)
		{
		...
		public async Task<SolutionZipArchive> DownloadAsync(CancellationToken cancellationToken = default)
		{
			if (solution.IsDeleted)
				throw new InvalidOperationException("Operation cannot be performed, the solution is deleted!");
			...
			try
			{
				...
				var response = (ExportSolutionResponse)await crm.ExecuteAsync(request, cancellationToken).ConfigureAwait(false);
				sw.Stop();
		...
		public Task UploadAndPublishAsync(byte[] zipFile, params string[] tableNames)
			=> UploadAndPublishAsync(zipFile, default, tableNames);

		public async Task UploadAndPublishAsync(byte[] zipFile, CancellationToken cancellationToken, params string[] tableNames)
		{
			if (solution.IsDeleted)
				throw new InvalidOperationException("Operation cannot be performed, the solution is deleted!");
			var sw = Stopwatch.StartNew();
			try
			{
				...
				await crm.ExecuteAsync(request2, cancellationToken).ConfigureAwait(false);
				sw.Stop();
				output.WriteLine("DONE in " + sw.Elapsed, ConsoleColor.Green);
			}
			catch
		...
		public async Task DeleteAsync(CancellationToken cancellationToken = default)
		{
			...
			try
			{
				...
				await solution.DeleteAsync(crm, cancellationToken).ConfigureAwait(false);
				output.WriteLine("Done", ConsoleColor.Green);
			}
			catch (Exception ex)
			{
