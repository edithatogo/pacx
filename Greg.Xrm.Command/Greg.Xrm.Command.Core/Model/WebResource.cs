...
namespace Greg.Xrm.Command.Model
{
	public class WebResource : EntityWrapper
	{
		...
		public async Task<string> GetContentAsync(IOrganizationServiceAsync2 crm, CancellationToken cancellationToken = default)
		{
			if (this.IsNew)
				return this.content;
			...
			var entity = await crm.RetrieveAsync(this.EntityName, this.Id, new ColumnSet("content"), cancellationToken).ConfigureAwait(false);
		...
		public class Repository : IWebResourceRepository
		{
			...
			public async Task<List<WebResource>> GetByNameAsync(IOrganizationServiceAsync2 crm, string[] fileNames, bool fetchContent = false, CancellationToken cancellationToken = default)
			{
			...
			public async Task<List<WebResource>> GetBySolutionAsync(IOrganizationServiceAsync2 crm, string solutionUniqueName, bool fetchContent = false, CancellationToken cancellationToken = default)
			{
