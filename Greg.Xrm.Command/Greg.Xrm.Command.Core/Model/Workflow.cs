...
#pragma warning disable IDE1006 // Naming Styles
	public class Workflow : EntityWrapper
	{
		...
		{
			public async Task<IReadOnlyList<Workflow>> GetByIdsAsync(IOrganizationServiceAsync2 crm, IEnumerable<Guid> ids, CancellationToken cancellationToken = default)
			{
			...
			public async Task<IReadOnlyList<Workflow>> GetByNameAsync(IOrganizationServiceAsync2 crm, string uniqueName, CancellationToken cancellationToken = default)
			{
				var query = new QueryExpression("workflow");
				query.ColumnSet.AddColumns(nameof(name), nameof(category), nameof(xaml), nameof(statecode), nameof(statuscode), nameof(triggeronupdateattributelist));
			...
			public async Task<IReadOnlyList<Workflow>> GetBySolutionAsync(IOrganizationServiceAsync2 crm, string solutionUniqueName, CancellationToken cancellationToken = default)
			{
				var query = new QueryExpression("workflow");
				query.ColumnSet.AddColumns(nameof(name), nameof(category), nameof(xaml), nameof(statecode), nameof(statuscode), nameof(triggeronupdateattributelist));
			...
			public async Task<IReadOnlyList<Workflow>> SearchByNameAndSolutionAndCategoryAsync(IOrganizationServiceAsync2 crm, string namePart, string? solutionUniqueName, Category? category, CancellationToken cancellationToken = default)
			{
				var query = new QueryExpression("workflow");
				query.ColumnSet.AddColumns(nameof(name), nameof(category), nameof(xaml), nameof(statecode), nameof(statuscode), nameof(triggeronupdateattributelist));
