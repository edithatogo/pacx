...
namespace Greg.Xrm.Command.Services.AttributeDeletion
{
	public abstract class AttributeDeletionStrategyBase : IAttributeDeletionStrategy
	{
		...
		public async Task HandleAsync(IOrganizationServiceAsync2 crm, AttributeMetadata attribute, DependencyList dependencies, CancellationToken cancellationToken = default)
		{
			var dependeciesFiltered = dependencies.OfType(this.ComponentType);
			if (dependeciesFiltered.Count == 0) return;

			await this.HandleInternalAsync(crm, attribute, dependeciesFiltered, cancellationToken).ConfigureAwait(false);
		}

		protected abstract Task HandleInternalAsync(IOrganizationServiceAsync2 crm, AttributeMetadata attribute, IReadOnlyList<Dependency> dependencies, CancellationToken cancellationToken);
