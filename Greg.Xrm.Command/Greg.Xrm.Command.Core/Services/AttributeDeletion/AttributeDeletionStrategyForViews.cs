...
namespace Greg.Xrm.Command.Services.AttributeDeletion
{
		...
		: AttributeDeletionStrategyBase
	{
			...
			CancellationToken cancellationToken)
		{
			...
			var savedQueries = await savedQueryRepository.GetByIdAsync(crm, dependencies.Select(x => x.dependentcomponentobjectid)).ConfigureAwait(false);
			viewList.AddRange(savedQueries);

			var userQueries = await userQueryRepository.GetContainingAsync(crm, attribute.EntityLogicalName, attribute.SchemaName).ConfigureAwait(false);
			...
			foreach (var e in viewList)
			{
				...
				await e.SaveOrUpdateAsync(crm, cancellationToken).ConfigureAwait(false);
