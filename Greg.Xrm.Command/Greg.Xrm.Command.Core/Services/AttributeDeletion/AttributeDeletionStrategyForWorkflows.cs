...
namespace Greg.Xrm.Command.Services.AttributeDeletion
{
		...
		: AttributeDeletionStrategyBase
	{
		...
		{
			var workflowList = await workflowRepository.GetByIdsAsync(crm, dependencies.Select(x => x.dependentcomponentobjectid), cancellationToken).ConfigureAwait(false);
			...
			foreach (var workflow in workflowList)
			{
				...
				try
				{
					var triggers = await processTriggerRepository.GetByWorkflowIdAsync(crm, workflow.Id, cancellationToken).ConfigureAwait(false);
					foreach (var trigger in triggers)
					{
						await crm.DeleteAsync(trigger.EntityName, trigger.Id, cancellationToken).ConfigureAwait(false);
					}
					...
					await clone.SaveOrUpdateAsync(crm, cancellationToken).ConfigureAwait(false);
				}
				finally
				{
					if (activate)
