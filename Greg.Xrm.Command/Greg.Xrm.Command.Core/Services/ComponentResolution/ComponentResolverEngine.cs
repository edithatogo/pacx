...
namespace Greg.Xrm.Command.Services.ComponentResolution
{
	public class ComponentResolverEngine : IComponentResolverEngine
	{
		...
		public async Task ResolveAllAsync(IReadOnlyCollection<SolutionComponent> componentList, IOrganizationServiceAsync2 crm, CancellationToken cancellationToken = default)
		{
			...
			foreach (var group in groups)
			{
				if (resolverStrategyCache.TryGetValue(group.Key, out var resolverFactory))
				{
					...
					await resolver.ResolveAsync([.. group], crm, cancellationToken).ConfigureAwait(false);
					continue;
				}
			...
			await entityAndAttributeResolver.ResolveAsync(componentList, crm, cancellationToken).ConfigureAwait(false);
		}
