...
namespace Greg.Xrm.Command.Services.ComponentResolution
{
	public class ResolverByQuery : IComponentResolver
	{
		...
		public async Task ResolveAsync(IReadOnlyCollection<SolutionComponent> componentList, IOrganizationServiceAsync2 crm, CancellationToken cancellationToken = default)
		{
			...
			var dict = await GetNamesAsync(crm, idList, cancellationToken).ConfigureAwait(false);
		...
		private async Task<Dictionary<Guid, string>> GetNamesAsync(IOrganizationServiceAsync2 crm, IReadOnlyList<Guid> componentIdSet, CancellationToken cancellationToken)
		{
