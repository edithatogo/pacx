...
namespace Greg.Xrm.Command.Services.ComponentResolvers
{
	public class ResolverForSystemForms : IComponentResolver
	{
		...
		public async Task<Dictionary<Guid, string>> GetNamesAsync(IReadOnlyList<Guid> componentIdSet, CancellationToken cancellationToken = default)
		{
			...
			try
			{
				...
				var result = await this.crm.RetrieveMultipleAsync(query, cancellationToken).ConfigureAwait(false);
