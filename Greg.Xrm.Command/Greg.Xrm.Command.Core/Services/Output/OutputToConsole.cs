...
namespace Greg.Xrm.Command.Services.Output
{
	public class OutputToConsole : IOutput
	{
		...
		public OutputToConsole(bool colorsEnabled = true)
		{
		...
		public IOutput Write(object? text)
		{
			lock (syncRoot)
			{
				Console.Write(OutputRedactor.Redact(text));
			}
			return this;
		}
		...
		public IOutput WriteLine(object? text)
		{
			lock (syncRoot)
			{
				Console.WriteLine(OutputRedactor.Redact(text));
			}
			return this;
		}
		...
		{
			this.WriteLine($"CorrelationId: {correlationId}", ConsoleColor.DarkGray);
			return this;
		}
	}
}
