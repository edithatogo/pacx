...
namespace Greg.Xrm.Command.Services.Plugin
{
	public interface ISdkMessageProcessingStepRepository
	{
		...
		Task<SdkMessageProcessingStep?> GetByIdAsync(IOrganizationServiceAsync2 crm, Guid id, CancellationToken cancellationToken = default);
		Task<SdkMessageProcessingStep[]> GetByKeyAsync(IOrganizationServiceAsync2 crm, PluginType pluginType, string messageName, string? primaryEntityName, PluginRegistrationToolkit.Stage? stage = null, CancellationToken cancellationToken = default);
