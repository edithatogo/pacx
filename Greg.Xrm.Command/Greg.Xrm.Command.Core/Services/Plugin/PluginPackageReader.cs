...
using PackagingPackage = System.IO.Packaging.Package;
...
namespace Greg.Xrm.Command.Services.Plugin
{
	...
	) : IPluginPackageReader
	{
		...
		public PluginPackageReadResult ReadPackageFile(string filePath)
		{
			...
			using (var package = PackagingPackage.Open(filePath, FileMode.Open, FileAccess.Read))
			{
		...
		public async Task<PluginAssemblyReadResult> ReadAssemblyFileAsync(string filePath, CancellationToken cancellationToken = default)
		{
			// Step 1: Validate input parameters
			if (string.IsNullOrWhiteSpace(filePath))
			{
			...
			try
			{
				var frameworkPath = await settingsRepository.GetAsync<string>(Constants.FrameworkPathKey, cancellationToken).ConfigureAwait(false);
