...
namespace Greg.Xrm.Command.Services.Plugin
{
	public class SdkMessageProcessingStep : EntityWrapper
	{
		...
		public class Repository : ISdkMessageProcessingStepRepository
		{
			...
			public async Task<SdkMessageProcessingStep?> GetByIdAsync(IOrganizationServiceAsync2 crm, Guid id, CancellationToken cancellationToken = default)
			{
			...
			public async Task<SdkMessageProcessingStep[]> GetByKeyAsync(IOrganizationServiceAsync2 crm, PluginType pluginType, string messageName, string? primaryEntityName, PluginRegistrationToolkit.Stage? stage, CancellationToken cancellationToken = default)
			{
				var query = new QueryExpression("sdkmessageprocessingstep");
				query.ColumnSet.AddColumns(columns);
				...
				var sdkMessageProcessingStepList = await crm.RetrieveMultipleAsync(query, cancellationToken).ConfigureAwait(false);
			...
			public async Task<SdkMessageProcessingStep[]> SearchByNameAsync(IOrganizationServiceAsync2 crm, string name, ConditionOperator op, bool includeInternalStages, CancellationToken cancellationToken)
			{
				...
				return await crm.RetrieveAllAsync(query, x => new SdkMessageProcessingStep(x), cancellationToken).ConfigureAwait(false);
			}
		}
	}
}
