...
namespace Greg.Xrm.Command.Services.Project
{
	public class PacxProjectRepository(ILogger<PacxProjectRepository> logger) : IPacxProjectRepository
	{
		...
		public async Task<PacxProjectDefinition?> GetCurrentProjectAsync(CancellationToken cancellationToken = default)
		{
			...
			try
			{
				var text = await File.ReadAllTextAsync(projectFile.FullName, cancellationToken).ConfigureAwait(false);
		...
		public async Task SaveAsync(PacxProjectDefinition projectDefinition, string folder, CancellationToken cancellationToken)
		{
			...
			await File.WriteAllTextAsync(file, projectString, cancellationToken).ConfigureAwait(false);
		}
	}
}
