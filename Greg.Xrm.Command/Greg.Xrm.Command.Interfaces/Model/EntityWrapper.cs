...
namespace Greg.Xrm.Command.Model
{
	...
	public abstract class EntityWrapper : IEntityWrapperInternal
	{
		...
		/// A value indicating whether the current entity has been correcly saved or updated.
		/// </returns>
		public virtual async Task<bool> SaveOrUpdateAsync(IOrganizationServiceAsync2 service, CancellationToken cancellationToken = default)
		{
			if (!IsChangeTrackingEnabled)
			{
				throw new InvalidOperationException("Change tracking not enabled, unable to save the record");
			...
			if (IsNew) // isNew
			{
				target.Id = await service.CreateAsync(target, cancellationToken);
				preImage.Id = target.Id;
			}
			else if (target.Attributes.Count > 0)
			{
				await service.UpdateAsync(target, cancellationToken);
			}
		...
		/// </summary>
		/// <param name="service">The organization service to be used to save or update the changes.</param>
		public virtual async Task DeleteAsync(IOrganizationServiceAsync2 service, CancellationToken cancellationToken = default)
		{
			if (IsNew)
				return;

			await service.DeleteAsync(this.EntityName, this.Id, cancellationToken);
			IsDeleted = true;
		}
		...
		{
			if (!string.Equals(other.EntityName, EntityName, StringComparison.Ordinal))
