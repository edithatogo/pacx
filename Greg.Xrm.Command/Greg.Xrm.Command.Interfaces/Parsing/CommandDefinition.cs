...
namespace Greg.Xrm.Command.Parsing
{
	...
	{
		private static readonly char[] MultiValueSeparators = ['\x1F', ','];
		...
		{
			var commandOptions = new Dictionary<string, string>(options, StringComparer.OrdinalIgnoreCase);
			var globalOptions = GlobalCommandOptions.Extract(commandOptions);
			...
			ApplyGlobalOptions(command, globalOptions);
			...
			foreach (var optionDef in this.Options)
			{
				...
				if (commandOptions.TryGetValue("--" + option.LongName, out var optionValue))
				{
					usedOptions.Add("--" + option.LongName);
				}
				else if (!string.IsNullOrWhiteSpace(option.ShortName) && commandOptions.TryGetValue("-" + option.ShortName, out optionValue))
				{
				...
				if ((propertyType == typeof(bool) || nullableUnderlyingType == typeof(bool)) && optionValue == string.Empty)
				{
					optionValue = "true";
				}
				...
				if (propertyType == typeof(List<string>) || propertyType == typeof(string[]))
				{
					var values = SplitMultiValue(optionValue);
					property.SetValue(command, propertyType == typeof(string[]) ? values.ToArray() : values);
					continue;
				}
			...
			if (usedOptions.Count != commandOptions.Count)
			{
				var unusedOptions = commandOptions.Keys.Except(usedOptions).ToList();
		...
		private static void ApplyGlobalOptions(object command, IReadOnlyDictionary<string, string> globalOptions)
		{
			if (globalOptions.TryGetValue("--format", out var format) && !string.IsNullOrWhiteSpace(format))
			{
				SetPropertyIfPresent(command, "Format", format);
			}

			if (globalOptions.TryGetValue("--profile", out var profile) && !string.IsNullOrWhiteSpace(profile))
			{
				SetPropertyIfPresent(command, "Profile", profile);
			}

			if (globalOptions.ContainsKey("--verbose"))
			{
				SetPropertyIfPresent(command, "Verbose", true.ToString(CultureInfo.InvariantCulture));
			}

			if (globalOptions.ContainsKey("--quiet"))
			{
				SetPropertyIfPresent(command, "Quiet", true.ToString(CultureInfo.InvariantCulture));
			}

			if (globalOptions.ContainsKey("--watch"))
			{
				SetPropertyIfPresent(command, "Watch", true.ToString(CultureInfo.InvariantCulture));
			}
		}

		private static void SetPropertyIfPresent(object command, string propertyName, string value)
		{
			var property = command.GetType().GetProperty(propertyName, BindingFlags.Public | BindingFlags.Instance | BindingFlags.IgnoreCase);
			if (property == null || !property.CanWrite) return;

			var convertedValue = Convert(value, property.PropertyType, propertyName, false, null);
		...
		private static List<string> SplitMultiValue(string optionValue)
		{
			return optionValue
		...
		private static object? Convert(string optionValue, Type propertyType, string argumentName, bool isRequired, object? defaultValue)
		{
			...
			var nullableUnderlyingType = Nullable.GetUnderlyingType(propertyType);
			if ((propertyType == typeof(bool) || nullableUnderlyingType == typeof(bool)) && optionValue == string.Empty) optionValue = "true";
			...
			var undelyingType = nullableUnderlyingType;
			...
			try
			{
				...
				var actualValue = System.Convert.ChangeType(optionValue, propertyType, CultureInfo.InvariantCulture);
				if (actualValue != null)
					return actualValue;
