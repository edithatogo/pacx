...
{
	sealed class CommandRunnerInteractive(
		IOutput output,
		...
		CommandRunnerBase(output, log, commandExecutorFactory, historyTracker, args, telemetryService, correlationIdProvider), ICommandRunner
	{
		...
		public async Task<int> RunCommandAsync(CancellationToken cancellationToken)
		{
			...
			while (true)
			{
				...
				var prompt = new TextPrompt<string>("Press [yellow]Enter[/] to run another command, or CTRL+C to exit.")
					.AllowEmpty();
				await console.PromptAsync(prompt, cancellationToken);
			}
		}
		...
		object TryInitializeCommand(CommandDefinition commandDefinition)
		{
			...
			return commandDefinition.CreateCommand(dict);
		}
		...
		private void ShowOptionHelp(OptionDefinition option)
		{
			...
			content.AppendLine();
			content.Append(string.Concat("[", DefaultColors.Text, "]Option:[/] [", DefaultColors.Command, "]--", Markup.Escape(option.Option.LongName), "[/]"));
			if (option.Option.ShortName is not null)
			{
				content.Append(string.Concat(", [", DefaultColors.Command, "]-", Markup.Escape(option.Option.ShortName), "[/]"));
			}

			if (option.Option.DefaultValue is not null)
			{
				content.AppendLine();
				content.Append(string.Concat("[", DefaultColors.Text, "]Default:[/] [", DefaultColors.Command, "]", Markup.Escape(option.Option.DefaultValue.ToString() ?? string.Empty), "[/]"));
			}
			...
			if (option.IsRequired)
			{
				content.Append(string.Concat("[", Color.Red, "]This option is required.[/]"));
			}
			else
			{
				content.Append(string.Concat("[", DefaultColors.Text, "]This option is optional.[/]"));
			}
		...
		private static IPrompt<string> CreatePrompt(OptionDefinition option, string promptText)
		{
			...
			{
				return CreateBoolPrompt(promptText);
			}
		...
		/// </summary>
		private static SelectionPrompt<string> CreateOptionsPrompt(OptionDefinition option, string promptText, Type enumType)
		{
		...
		/// </summary>
		private static SelectionPrompt<string> CreateBoolPrompt(string promptText)
		{
			var prompt = new SelectionPrompt<string>()
				.Title(promptText)
		...
		/// </summary>
		private static TextPrompt<string> CreateTextPrompt(OptionDefinition option, string promptText)
		{
