...
{
	static class VerbTreeRecourser
	{
		public static CommandDefinition? Recourse(IAnsiConsole console, IEnumerable<VerbNode> tree, IEnumerable<VerbNode> rootTree)
		{
			...
			do
			{
				var verbsToShow = tree
					.Where(x => !x.IsHidden && (x.Command is null || x.Command.CommandType.GetCustomAttribute<HideInInteractiveExperienceAttribute>() is null))
					.ToArray();
		...
		private static Stack<IEnumerable<VerbNode>> BuildParentStack(IEnumerable<VerbNode> tree, IEnumerable<VerbNode> rootTree)
		{
			...
			if (rootTree.Any(child => child.Children == tree))
			{
				parents.Push(rootTree);
				return parents;
			}

			foreach (var children in rootTree.Select(child => child.Children))
			{
				var result = BuildParentStack(tree, children);
				if (result.Count > 0)
				{
