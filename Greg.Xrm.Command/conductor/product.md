...
### Phase 13: Microsoft Release Plan Intelligence
| Command | Description | Priority | Source |
| :--- | :--- | :--- | :--- |
| `release-plan browse` | Browse Microsoft release-plan pages for Power Apps, Power Pages, Power Automate, AI Builder, Microsoft Dataverse, and Governance and administration. | **Medium** | `releaseplans.microsoft.com` |
| `release-plan watch` | Track snapshots for selected release-plan families and flag new or changed items. | **High** | `releaseplans.microsoft.com` |
| `release-plan diff` | Compare release-plan snapshots across dates or product families. | **High** | `releaseplans.microsoft.com` |
| `release-plan export` | Export release-plan deltas to markdown, JSON, or docs-ready summaries. | **Medium** | `releaseplans.microsoft.com` |

### Phase 14: Microsoft Forms Authoring
| Command | Description | Priority | API |
| :--- | :--- | :--- | :--- |
| `forms create` | Create a source-controlled Microsoft Forms authoring manifest. | **High** | Local manifest |
| `forms update` | Update manifest metadata and published state. | **High** | Local manifest |
| `forms delete` | Delete a source-controlled form manifest with confirmation safeguards. | **High** | Local manifest |
| `forms publish` | Publish or unpublish the manifest and control the local published flag. | **Medium** | Local manifest |
| `forms inspect` | Inspect a source-controlled manifest and summarize its contents. | **Low** | Local manifest |
| `forms question add` | Add a new question to the manifest. | **High** | Local manifest |
| `forms question list` | List questions in the manifest. | **Low** | Local manifest |
| `forms question update` | Update question text, type, options, and validation flags. | **High** | Local manifest |
| `forms question delete` | Remove a question from the manifest. | **Medium** | Local manifest |
| `forms section add` | Add a new section to the manifest. | **Medium** | Local manifest |
| `forms section list` | List sections in the manifest. | **Low** | Local manifest |
| `forms section delete` | Remove a section from the manifest. | **Medium** | Local manifest |
| `forms section update` | Update section ordering and prompts. | **Low** | Local manifest |
